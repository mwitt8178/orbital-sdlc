 ...
 */
export function checkIssue(personaId, scopes, ctx = {}) {
    const policy = getActivePolicy();
    for (const rule of policy.sod_rules) {
        if (!appliesTo(rule, personaId))
            continue;
        ...
        for (const combo of rule.forbidden_scope_combinations) {
            if (scopeListIncludes(scopes, combo.when_holds.scopeKey, combo.when_holds.pattern) &&
    ...
    if (scopes.ceremony_role.includes('chair') &&
 ...
 */
export function checkRuntime(bundle, tool, params) {
    // Verifier writing to a path it can read = SoD violation.
    if (bundle.persona_id === 'verifier' &&
...
// Helpers
// ---------------------------------------------------------------------------
function appliesTo(rule, personaId) {
    if (rule.applies_to_persona.includes('*'))
        return true;
    return rule.applies_to_persona.includes(personaId);
}
function scopeListIncludes(scopes, key, pattern) {
...
function matchesPattern(grantedPattern, forbiddenPattern) {
