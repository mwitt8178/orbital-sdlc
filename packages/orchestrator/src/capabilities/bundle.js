...
import { CapabilityBundleSchema, CapabilityBundleUnsignedSchema, } from '@orbital/types';
 ...
 */
export async function signBundle(unsigned, keyManager) {
...
export function bundleHash(unsigned) {
 ...
 */
export async function verifyBundle(bundle, keyManager, now = new Date(), opts = {}) {
...
async function loadPublicKey(keyId, keyManager) {
...
export async function isRevoked(capabilityId) {
    const rows = await db
 ...
 */
export async function verifyBundleSignatureOnly(bundle, publicKey) {
    const parsed = CapabilityBundleSchema.safeParse(bundle);
    if (!parsed.success) {
        return {
            ok: false,
