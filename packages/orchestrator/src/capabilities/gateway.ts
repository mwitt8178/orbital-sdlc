...
export const TOOL_TO_SCOPE_KEY: Record<string, ScopeKey> = {
...
export function extractTarget(_toolName: string, params: Record<string, unknown>): string {
  if (typeof params['path'] === 'string') return params['path']
  if (typeof params['channel'] === 'string') return params['channel']
  if (typeof params['target'] === 'string') return params['target']
  if (typeof params['url'] === 'string') return params['url']
  if (typeof params['host'] === 'string') return params['host']
  if (typeof params['secret_key'] === 'string') return params['secret_key']
  return _toolName
}
...
// ---------------------------------------------------------------------------
// Scope matching (per TRD-06 §6.2.2)
// ---------------------------------------------------------------------------
...
function matchScope(
  ...
  switch (scopeKey) {
    ...
    case 'files_write': {
      ...
      if (!norm) return null
      // Hard deny first.
      if (PATH_HARD_DENY.some((p) => micromatch.isMatch(norm, p, { dot: true }))) return null
    ...
    case 'secrets': {
      ...
      if (scopes.secrets.some((s) => s.includes('*'))) return null
    ...
    case 'git_commit': {
      ...
      if (!branch || !Array.isArray(pathsRaw)) return null
      const paths = pathsRaw.filter((p): p is string => typeof p === 'string')
      const matched = scopes.git_commit.find(
        (rule) =>
          micromatch.isMatch(branch, rule.branch) &&
...
function violatesHardProhibition(scopeKey: ScopeKey, params: Record<string, unknown>): boolean {
  if (scopeKey === 'secrets') {
    const k = stringParam(params, 'secret_key')
    if (k && k.includes('*')) return true
  }
...
function hostMatches(host: string, pattern: string): boolean {
  if (pattern === '*') return false // wildcard egress is hard-denied
...
function channelMatches(channel: string, pattern: string): boolean {
...
export function scopeKeyForTool(toolName: string): ScopeKey | undefined {
 ...
 */
export function gatewayValidateChannel(
  bundle: CapabilityBundle,
  scope: 'channel_read' | 'channel_post',
  channelName: string,
): boolean {
