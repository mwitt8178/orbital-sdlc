...
export class HookEngine {
     ...
     */
    registry = new Map();
    constructor(eventStore, db = defaultDb) {
    ...
    // ---------------------------------------------------------------------------
    async fire(eventType, payload, context, timing) {
        ...
        for (const hook of hooks) {
            ...
            void this.writeInvocationAndEvents(hook, eventType, timing, decision, durationMs, invocationId, payloadDigest, context).catch((writeErr) => {
                logger.error({ writeErr, hook_slug: hook.slug, invocationId }, 'HookEngine: failed to write invocation/events');
            });
            if (!decision.allow) {
    ...
    // Internal helpers
    // ---------------------------------------------------------------------------
    lookupHooks(eventType, timing) {
        const timingMap = this.registry.get(eventType);
        if (!timingMap)
            return [];
        return timingMap.get(timing) ?? [];
    }
    async writeInvocationAndEvents(hook, eventType, timing, decision, durationMs, invocationId, payloadDigest, context) {
        ...
        const actor = { type: 'hook', hook_id: hook.hook_id, hook_version: hook.hook_version_id };
