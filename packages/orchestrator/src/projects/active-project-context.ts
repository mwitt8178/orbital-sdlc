...
import type { ReqContext } from '../trpc/init.js'
 ...
 */
export function readActiveProjectIdFromHeaders(
 ...
 */
export function optionalActiveProject(ctx: ReqContext): string | null {
 ...
 */
export function requireActiveProject(ctx: ReqContext): string {
  const id = optionalActiveProject(ctx)
  if (!id) {
    throw new OrbitalError(
      PROJECTS_ERROR_CODES.ACTIVE_PROJECT_REQUIRED,
      `Missing ${ACTIVE_PROJECT_HEADER} header; client must select a project before calling this procedure.`,
    )
  }
  return id
}
