...
import { personaOfRecordLinks } from '../db/schema/uat.js'
import { tasks, worktrees } from '../db/schema/orchestration.js'
import { capabilityGrants } from '../db/schema/capabilities.js'
...
import {
  buildPersonaOfRecordSystemPrompt,
  buildPersonaOfRecordUserPrompt,
  PersonaOfRecordResponseSchema,
  type PersonaOfRecordContext,
} from '../personas/prompts/persona-of-record.js'
...
export class DefaultPersonaOfRecord implements PersonaOfRecord {
  constructor(
     ...
     */
    private readonly driver: AnthropicDriver | null = null,
  ) {}

  async resolve(params: PersonaOfRecordResolveParams): Promise<string> {
    ...
    const acScopedLinks = await this.db
      ...
      .where(
        and(
          eq(personaOfRecordLinks.storyId, storyId),
          eq(personaOfRecordLinks.acId, acId),
          eq(personaOfRecordLinks.role, 'implementation'),
        ),
      )
      .limit(1)
    ...
    if (acScopedLinks.length > 0 && acScopedLinks[0]) {
      logger.debug(
        { acId, storyId, personaId: acScopedLinks[0].personaId },
        'PersonaOfRecord: resolved via AC-scoped link (step 1)',
      )
    ...
    if (storyScopedLinks.length === 1 && storyScopedLinks[0]) {
      logger.debug(
        { storyId, personaId: storyScopedLinks[0].personaId },
        'PersonaOfRecord: resolved via story-scoped implementation link (step 2)',
      )
    ...
    if (taskRows.length > 0 && taskRows[0]) {
      ...
      if (this.driver && defectDescription) {
        const reasoned = await this._reasonViaDriver({
          ...
          if (err instanceof AnthropicDriverNoKeyError) {
            logger.debug(
              { acId, storyId },
              'PersonaOfRecord: ANTHROPIC_API_KEY unset; using tasks.persona_id',
            )
          } else {
            logger.warn(
              { err, acId, storyId },
              'PersonaOfRecord: driver reasoning failed; using tasks.persona_id',
            )
          }
          return null
        })
      ...
      logger.debug(
        { storyId, taskId: taskRows[0].taskId, personaId: tasksFallbackPersonaId },
        'PersonaOfRecord: resolved via most-recent task persona_id (step 3)',
      )
    ...
    // -----------------------------------------------------------------------
    logger.warn(
      { acId, storyId, sessionId },
      'PersonaOfRecord: unresolvable — falling back to sentinel persona:unknown',
    )
  ...
  private async _reasonViaDriver(params: {
    acId: string
    storyId: string
    sessionId: string
    traceId: string
  ...
  }): Promise<{ persona_id: string; confidence_score: number } | null> {
    if (!this.driver) {
    ...
    const recentCommits: PersonaOfRecordContext['recentCommits'] = []
    ...
    if (taskRow[0]?.declaredWritePaths) {
      const paths = taskRow[0].declaredWritePaths as string[]
      for (const p of paths.slice(0, 10)) filePaths.push(p)
    }
      ...
      if (wtPath && filePaths.length > 0) {
        try {
          const log = execFileSync(
            'git',
            ['log', '-5', '--format=%H%x09%an%x09%s', '--', ...filePaths],
            { cwd: wtPath, timeout: 3000, encoding: 'utf8' },
          )
    ...
    const ctx: PersonaOfRecordContext = {
      acId: params.acId,
    ...
    if (!candidatePersonaSlugs.includes(r.persona_id)) {
      logger.warn(
        { suggested: r.persona_id, candidates: candidatePersonaSlugs },
        'PersonaOfRecord: LLM suggested an unknown persona; deferring to fallback',
      )
      return null
    }
...
export function createPersonaOfRecord(
  db: DB,
...
// =============================================================================
// Attestation-chain walk (TRD-11 §7.2, SAO §5.9)
// =============================================================================
 ...
 */
export async function resolvePersonaOfRecord(
  taskId: string,
  filePaths: string[],
  db: DB,
  traceId?: string,
  ...
  // -------------------------------------------------------------------------
  const results: AttestationChainResult[] = []
...
async function resolveOneFilePath(params: {
  ...
  if (commitHash !== null) {
    ...
    logger.debug(
      { taskId, filePath, commitHash, traceId },
      'resolvePersonaOfRecord: no CommitSigned event for commit — falling through to capability',
    )
  }
  ...
  if (currentCapabilityId !== null) {
    ...
    logger.debug(
      { taskId, filePath, currentCapabilityId, traceId },
      'resolvePersonaOfRecord: capability_grant not found — falling through to task persona',
    )
  }
  ...
  if (personaId !== null && personaId !== undefined) {
    logger.debug(
      { taskId, filePath, personaId, traceId },
      'resolvePersonaOfRecord: resolved via task persona_id (step 4)',
    )
    return {
      filePath,
      personaId,
  ...
  logger.warn(
    { taskId, filePath, traceId },
    'resolvePersonaOfRecord: all steps failed — sentinel',
  )
  return {
    filePath,
    personaId: PERSONA_OF_RECORD_UNKNOWN,
...
function gitLastCommit(
  ...
  try {
    ...
    if (!output || output.length !== 40) {
      ...
      logger.debug(
        { taskId, filePath, worktreePath, traceId },
        'resolvePersonaOfRecord: git log returned no commit for file',
      )
      return null
    }
    ...
    logger.warn(
      { err, taskId, filePath, worktreePath, traceId },
      'resolvePersonaOfRecord: git log failed for file — skipping step 1',
    )
    return null
  }
}
 ...
 */
async function lookupCommitSignedEvent(
  commitHash: string,
  db: DB,
  traceId: string,
): Promise<{ personaId: string; eventId: string } | null> {
  try {
    const rows = await db
      .select({
      ...
      .where(
        and(
          ...
          dSQL`${events.payload}->>'commit_hash' = ${commitHash}`,
        ),
      )
      .limit(1)
    ...
    logger.debug(
      { commitHash, actorType: actor['type'], traceId },
      'resolvePersonaOfRecord: CommitSigned event found but actor is not persona type',
    )
    return null
  } catch (err) {
    logger.warn(
      { err, commitHash, traceId },
      'resolvePersonaOfRecord: error querying CommitSigned event — skipping step 2',
    )
    return null
  }
}
 ...
 */
async function lookupCapabilityGrant(
  capabilityId: string,
  db: DB,
  traceId: string,
  ...
  try {
    ...
    logger.warn(
      { err, capabilityId, traceId },
      'resolvePersonaOfRecord: error querying capability_grants — skipping step 3',
    )
    return null
  }
}
