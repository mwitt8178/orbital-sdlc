...
import { defects, defectLineage, uatSessions } from '../db/schema/uat.js'
import { tasks } from '../db/schema/orchestration.js'
import { UAT_ERROR_CODES, type DefectRow, type DefectSeverity, type CreateDefectParams } from './types.js'
...
const SEVERITY_CRITICAL_PATTERN =
  /\b(security|data loss|financial|production outage)\b/i

// ---------------------------------------------------------------------------
// submitDefect params (Round 6 #3)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// getDefectsForTask params (Round 6 #3)
// ---------------------------------------------------------------------------
...
export interface DefectService {
   ...
   */
  promoteToBacklog(defectId: string, epicId: string): Promise<{ storyId: string }>
...
export class DefaultDefectService implements DefectService {
  constructor(
  ...
  async createDefect(params: CreateDefectParams): Promise<DefectRow> {
    const {
      failedAcResultId,
      sessionId,
      storyId,
    ...
    const existing = await this.db
      ...
      .where(
        and(
          eq(defects.originStoryId, storyId),
          eq(defects.originAcId, acId),
          inArray(defects.state, ['open', 'triaged', 'assigned', 'in_progress']),
        ),
      )
      .limit(1)
    ...
    if (existing.length > 0 && existing[0]) {
      ...
      logger.info(
        { defectId: existingDefect.defectId, reopenCount: newReopenCount },
        'DefectService: updated existing defect (reopened)',
      )
    ...
    logger.info(
      { defectId, defectKey, severity, storyId, acId },
      'DefectService: created new defect',
    )
  ...
  async submitDefect(params: SubmitDefectParams): Promise<SubmitDefectResult> {
    const {
      taskId,
      acId,
      acText,
    ...
    if (!taskRow) {
      throw new OrbitalError(
        UAT_ERROR_CODES.NOT_FOUND_DEFECT,
    ...
    logger.info(
      { defectId, defectKey, taskId, acId, severity, iterationNumber },
      'DefectService: operator defect submitted',
    )
  ...
  async getDefectsForTask(taskId: string): Promise<DefectHistoryEntry[]> {
  ...
  async markDefectFixed(defectId: string, resolvedBy: string): Promise<void> {
    const now = new Date()
    const traceId = uuidv7()
  ...
  async promoteToBacklog(defectId: string, epicId: string): Promise<{ storyId: string }> {
    const [defect] = await this.db
      .select()
      .from(defects)
      .where(eq(defects.defectId, defectId))
    ...
    logger.info(
      { defectId, storyId: story.storyId, epicId },
      'DefectService: promoted defect to backlog story',
    )
  ...
  assignSeverity(params: {
    ...
    const {
      acText,
      failedAcCountForStory,
      totalAcCountForStory,
      isInActiveSprint,
      isReopen,
      acTags = [],
      observedValue,
    ...
    const hasPerformanceTag =
      acTags.some((t) => t === 'performance' || t === 'latency') ||
...
export function createDefectService(
  db: DB,
