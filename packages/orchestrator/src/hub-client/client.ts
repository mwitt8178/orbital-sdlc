...
export interface HubClient {
  ...
  workers: {
    ...
    updateState(
      ...
      workerId: string,
      state: string,
    ): Promise<HubProxyResult<HubWorkerRegistration>>
  }
...
class HubClientImpl implements HubClient {
  private _status: HubStatus
  // Round 7-06: connection state machine
  private _connectionState: HubConnectionState = 'connected'
  private _connectionStateSource: (() => HubConnectionState) | null = null
  ...
  readonly tasks: HubClient['tasks'] = {
    list: (tenantId, sprintId) =>
      this.rpc<HubTask[]>('orchestration.tasks.list', { sprintId }, tenantId),

    get: (tenantId, taskId) =>
      this.rpc<HubTask | null>('orchestration.tasks.get', { taskId }, tenantId),

    claim: (tenantId, taskId, installId) =>
      this.rpc<HubTask>('orchestration.tasks.claim', { taskId, installId }, tenantId),

    updateState: (tenantId, taskId, state) =>
      this.rpc<HubTask>('orchestration.tasks.updateState', { taskId, state }, tenantId),
  }
  ...
  readonly events: HubClient['events'] = {
    append: (event) =>
      this.rpc<HubEventEnvelope>('audit.events.append', event, event.tenant_id),
  }
  ...
  readonly workers: HubClient['workers'] = {
    register: (reg) =>
      this.rpc<HubWorkerRegistration>('orchestration.workers.register', reg, reg.tenant_id),

    updateState: (tenantId, workerId, state) =>
      this.rpc<HubWorkerRegistration>(
        'orchestration.workers.updateState',
        { workerId, state },
        tenantId,
      ),
  }
  ...
  async query<T>(procedure: string, input: unknown, tenantId: string): Promise<HubProxyResult<T>> {
    return this.rpc<T>(procedure, input, tenantId)
  }

  async mutate<T>(procedure: string, input: unknown, tenantId: string): Promise<HubProxyResult<T>> {
    return this.rpc<T>(procedure, input, tenantId)
  }
  ...
  private async rpc<T>(
    ...
    try {
      ...
      if (err instanceof LocalDataLeakError) {
        logger.fatal(
          { procedure, path: err.path, reason: err.reason },
          'hub-client: BLOCKED outgoing request — local-only data detected',
        )
    ...
    try {
      ...
      const data = extractTrpcData<T>(parsed)
  ...
  private markStatus(status: HubStatus['status'], errorMessage: string | null): void {
    this._status = {
      status,
...
function extractErrorMessage(parsed: unknown): string {
  if (typeof parsed !== 'object' || parsed === null) return 'Unknown hub error'
...
function extractTrpcData<T>(parsed: unknown): T {
  ...
  if (Array.isArray(parsed) && parsed.length > 0) {
    ...
    if (result) {
      ...
      if (data && 'json' in data) return data['json'] as T
 ...
 */
export function createHubClient(): HubClient | null {
  const env = loadEnv()
  if (!env.ORBITAL_HUB_URL) return null
 ...
 */
export function createHubClientForTest(hubUrl: string, tenantId: string): HubClient {
