 ...
 * isolation.
 */
import { HEADER_INSTALL_ID, HEADER_SIG, HEADER_SIG_BODY, signEnvelope, } from '../keys/envelope.js';
 ...
 */
export function _setInstallKeyPath(filePath) {
 ...
 */
export function _resetCachedInstallKey() {
    _cachedKey = null;
}
async function getKey() {
    if (_cachedKey)
        return _cachedKey;
    _cachedKey = await getOrCreateInstallKey(_customKeyPath ? { filePath: _customKeyPath } : {});
    // Log only safe fingerprint bytes
    const fingerprint = Buffer.from(_cachedKey.publicKey).toString('hex').slice(0, 16);
    logger.info({ installId: _cachedKey.installId, fingerprint }, 'hub-client/auth: install key loaded');
    return _cachedKey;
}
/**
 ...
 */
export async function buildSignedHeaders(method, bodyBytes) {
 ...
 * calling fetch().
 */
export async function attachAuthHeaders(headers, method, bodyBytes) {
 ...
 */
export async function getLocalInstallId() {
    const key = await getKey();
 ...
 */
export async function getLocalPublicKeyB64() {
    const key = await getKey();
