...
// Round 7-06: connection state for outbox bridge
...
class HubWsClientImpl implements HubWsClient {
  private ws: WebSocket | null = null
  private stopped = false
  private reconnectTimer: ReturnType<typeof setTimeout> | null = null
  private reconnectDelayMs: number
  private _status: WsClientStatus = 'disconnected'
  private _lastSeenEventId: string | null = null

  /** All active subscriptions. Map from pattern → Set<handler>. */
  private readonly subscriptions = new Map<string, Set<HubWsEventHandler>>()

  constructor(private readonly opts: HubWsClientOptions) {
  ...
  private unsubscribeHandler(pattern: string, handler: HubWsEventHandler): void {
    const handlers = this.subscriptions.get(pattern)
    if (handlers) {
  ...
  private sendSubscribe(patterns: string[], cursor: string | null): void {
  ...
  private scheduleReconnect(): void {
    ...
    this.reconnectDelayMs = Math.min(
      delay * 2,
      this.opts.maxReconnectMs ?? DEFAULT_MAX_RECONNECT_MS,
    )
...
function patternMatchesEvent(pattern: string, event: EventEnvelope): boolean {
  ...
  if (pattern.startsWith('worker:') && pattern.endsWith(':*')) {
    ...
    return (
      ...
      (payload['install_id'] === installId ||
        (event.actor as Record<string, unknown>)?.['install_id'] === installId)
    )
  }
  if (pattern === 'team:presence') {
    return (event as unknown as Record<string, unknown>)['aggregate_type'] === 'presence' ||
...
export function createHubWsClient(opts: HubWsClientOptions): HubWsClient {
 ...
 */
export function createHubWsClientFromEnv(
  onStatusChange?: (status: WsClientStatus) => void,
): HubWsClient | null {
