 ...
 */
export function readIdempotencyKey(ctx) {
...
// ---------------------------------------------------------------------------
export async function readCached(db, idempotencyKey, route) {
    try {
      ...
      `);
        ...
        logger.warn({ err, route, idempotencyKey }, 'idempotency: cache read failed; proceeding to run mutation');
        return null;
    }
}
export async function writeCached(db, idempotencyKey, route, status, responseJson) {
    ...
    try {
      ...
      `);
        ...
        logger.warn({ err, route, idempotencyKey, status }, 'idempotency: cache write failed; mutation already executed');
    }
}
// ---------------------------------------------------------------------------
// Middleware factory
...
export function createIdempotencyMiddleware(deps) {
    ...
    return async function idempotencyMiddleware(opts) {
        ...
        if (cached) {
            logger.debug({ route, idempotencyKey: key, status: cached.status }, 'idempotency: returning cached response');
            if (cached.status === 'error') {
                const errPayload = cached.result;
                throw new TRPCError({
        ...
        const errEnvelope = err instanceof TRPCError
