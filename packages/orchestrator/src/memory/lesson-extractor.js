 ...
 */
export async function extractAndStoreLessons(params) {
    const { tenantId, projectId, taskId, personaSlug, taskTitle, taskDescription, workerOutput, db, eventStore, anthropicApiKey, } = params;
    if (!anthropicApiKey || anthropicApiKey.trim().length === 0) {
        logger.debug({ taskId, personaSlug }, 'lesson-extractor: ANTHROPIC_API_KEY not set, skipping lesson extraction');
        return 0;
    }
...
Persona that ran this task: ${personaSlug}`;
    ...
    try {
        ...
        const toolUseBlock = result.content.find((b) => b.type === 'tool_use' && b.name === 'extract_lessons');
        if (!toolUseBlock) {
    ...
    for (const lesson of response.lessons) {
        try {
            ...
            }, personaSlug, tenantId);
            written++;
            logger.info({ taskId, personaSlug, lessonTitle: lesson.title, tenantId, projectId }, 'lesson-extractor: lesson written to memory');
        }
        catch (err) {
...
//# sourceMappingURL=lesson-extractor.js.map