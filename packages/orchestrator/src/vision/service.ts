...
import type {
  ...
  VisionSessionId,
  VisionVersionId,
  VisionDocumentContent,
  VisionDocumentContentDraft,
} from './types.js'
...
export interface VisionService {
  ...
  draft(
    sessionId: VisionSessionId,
    content: VisionDocumentContentDraft,
    draftSummary: string,
    actor: Actor,
    traceId: string,
  ...
  lock(params: {
    ...
    changelog: string
    attestation: { no_edge_cases: boolean }
    actor: Actor
    traceId: string
    justification: string
  }): Promise<VisionDocumentVersion>
...
export class DefaultVisionService implements VisionService {
  constructor(
    ...
    private readonly capabilityAuthority: ICapabilityAuthority,
  ...
  async start(params: {
    ...
    const existing = await this.db
      ...
      .where(
        and(
          eq(visionDocuments.installId, this.installId),
          eq(visionDocuments.title, title),
        ),
      )
      .limit(1)
    ...
    if (existing[0]) {
      ...
      if (Date.now() - createdAt < 5 * 60 * 1000) {
        ...
        const session = await this.db
          ...
          .where(
            and(
              eq(visionSessions.visionDocumentId, existingDoc.visionDocumentId),
        ...
        if (session[0]) {
          return {
            vision_document_id: existingDoc.visionDocumentId as VisionDocumentId,
            vision_session_id: session[0].visionSessionId as VisionSessionId,
            pm_persona_request_id: uuidv7(),
          }
        }
      }
      throw new OrbitalError(
        'CONFLICT_DUPLICATE_VISION_TITLE',
        `A vision document with title '${title}' already exists in this installation.`,
        { title },
        'no_retry',
      )
    }

    const documentId = uuidv7() as VisionDocumentId
    const sessionId = uuidv7() as VisionSessionId
    ...
    await this.db.insert(visionDocuments).values({
      ...
      mondayItemId: null,
      createdBy: actor as unknown as Record<string, unknown>,
    ...
    await this.db.insert(visionSessions).values({
      ...
      pmCapabilityId: null,
      startedBy: actor as unknown as Record<string, unknown>,
    })
    ...
    const pmPersona = await resolvePersona(this.db, this.personaLoader, 'pm').catch(
      (err) => {
        logger.warn(
          { err },
          'VisionService.start: PM persona not found; proceeding without spawn',
        )
        return null
      },
    )
    ...
    const channelResult = await this.channelsService.ensureChannel(
      'sprint',
      `vision-intake-${sessionId}`,
      {
    ...
    logger.info(
      { channelId: channelResult.channelId, sessionId },
      'vision_intake_channel_ensured',
    )
    ...
    if (pmPersona) {
      try {
        ...
        await this.channelsService.post(
        ...
        if (this.scheduler) {
          ...
          try {
            ...
            void this.scheduler.tick().catch((err: unknown) => {
              logger.warn(
                { err, sessionId, documentId },
                'VisionService.start: immediate scheduler.tick failed; periodic loop will retry',
              )
            })
            logger.info(
              { sessionId, pmTaskId, documentId },
              'vision_pm_task_enqueued',
            )
          } catch (err) {
            logger.warn(
              { err, sessionId, documentId },
              'VisionService.start: failed to enqueue PM task — session open, capability issued, but PM persona will not run',
            )
          }
        } else {
          logger.warn(
            { sessionId },
            'VisionService.start: no Scheduler injected — PM persona session opened but no worker will spawn (boot DI gap)',
          )
        }

        logger.info(
          { sessionId, capabilityId: bundle.bundle.capability_id, personaId: pmPersona.personaId },
          'vision_pm_persona_spawned',
        )
      } catch (err) {
    ...
    if (initial_prompt && initial_prompt.trim().length > 0) {
      try {
        ...
        logger.warn(
          { err, sessionId },
          'VisionService.start: auto-send of initial_prompt failed; session open without first message',
        )
      }
    }
  ...
  async sendMessage(
    ...
    if (session.state !== 'open' && session.state !== 'closed_drafted') {
      throw new OrbitalError(
        ...
        `Vision session ${sessionId} is in state '${session.state}' and cannot accept messages.`,
        { state: session.state },
        'no_retry',
      )
    }
  ...
  async draft(
    ...
    if (!parseResult.success) {
      throw new OrbitalError(
        ...
        `Draft content validation failed: ${parseResult.error.message}`,
    ...
    const versionId = uuidv7() as VisionVersionId
    const contentHash = computeContentHash(content)
    const openQCount = Array.isArray((content as Record<string, unknown>)['open_questions'])
      ? ((content as Record<string, unknown>)['open_questions'] as unknown[]).length
      : 0
    ...
    await this.db.insert(visionVersions).values({
      ...
      isLocked: 0,
      draftedBy: actor as unknown as Record<string, unknown>,
    })
  ...
  async reviewDraft(documentId: VisionDocumentId): Promise<{
    vision_document_id: string
    current_version: VisionDocumentVersion | null
    blocking_open_questions: string[]
    missing_required_fields: string[]
    ...
    let currentVersion: VisionDocumentVersion | null = null
    let validationResult = { missing_fields: [] as string[], blocking_open_questions: [] as string[] }
    ...
    if (doc.currentVersionId) {
      ...
      if (v) {
        ...
        validationResult = validateForLock(v.content as Record<string, unknown>, { no_edge_cases: false })
      }
    }
  ...
  async lock(params: {
    ...
    assertLockAllowed(doc.lifecycleState as 'drafting' | 'locked' | 'revised' | 'abandoned', actor.type)

    if (!doc.currentVersionId) {
      throw new OrbitalError(
        'CONFLICT_NO_DRAFT_VERSION',
        'No draft version exists to lock.',
        {},
        'no_retry',
      )
    }
    ...
    const validation = validateForLock(currentVersion.content as Record<string, unknown>, attestation)
    if (!validation.ready) {
      ...
      if (validation.missing_fields.length > 0) {
        throw new OrbitalError(
          'VALIDATION_REQUIRED_FIELD_MISSING',
          `Lock rejected: missing required fields: ${validation.missing_fields.join(', ')}`,
          { missing_fields: validation.missing_fields },
          'no_retry',
        )
      }
      throw new OrbitalError(
        'CONFLICT_OPEN_QUESTIONS_BLOCK_LOCK',
        `Lock rejected: ${validation.blocking_open_questions.length} blocking open question(s) unresolved.`,
        { blocking_open_questions: validation.blocking_open_questions },
        'escalate_to_human',
      )
    }
    ...
    const hookDecision = await this.hookEngine.fire(
      'pre-status-transition',
      {
        aggregate_type: 'vision_document',
        aggregate_id: documentId,
        from_state: 'drafting',
    ...
    if (!hookDecision.allow) {
      ...
      throw new OrbitalError(
        'HOOK_REJECTED_PRE_LOCK',
        `Lock rejected by hook: ${hookDecision.reason}`,
        { hook_id: hookDecision.hook_id },
        'escalate_to_human',
      )
    }
    ...
    const lockedVersionId = uuidv7() as VisionVersionId
    ...
    await this.db.insert(visionVersions).values({
      ...
      content: currentVersion.content as Record<string, unknown>,
      contentHash,
      changelog,
      previousVersionId: currentVersion.visionVersionId,
      isLocked: 1,
      lockedAt: new Date(now),
      lockedBy: actor as unknown as Record<string, unknown>,
      lockEventId: lockEvent.event_id,
      draftedBy: currentVersion.draftedBy as Record<string, unknown>,
    })
    ...
    const updatedRows = await this.db
      ...
      .where(
        and(
          eq(visionDocuments.visionDocumentId, documentId),
          eq(visionDocuments.currentVersionNumber, doc.currentVersionNumber),
          eq(visionDocuments.lastEventId, doc.lastEventId),
        ),
      )
    ...
    if (updatedRows.length === 0) {
      throw new OrbitalError(
        'CONFLICT_OPTIMISTIC_LOCK_FAILED',
        `Concurrent modification detected: vision document ${documentId} was modified during lock. Retry the operation.`,
        { document_id: documentId, expected_version: doc.currentVersionNumber },
        'retry_now',
      )
    }
    ...
    await this.db
      ...
      .where(
        and(
          eq(visionSessions.visionDocumentId, documentId),
          eq(visionSessions.state, 'open'),
        ),
      )
  ...
  async revise(params: {
    ...
    assertReviseAllowed(doc.lifecycleState as 'drafting' | 'locked' | 'revised' | 'abandoned', actor.type)

    // Optimistic concurrency: base_version_id must equal current_version_id
    if (doc.currentVersionId !== baseVersionId) {
      throw new OrbitalError(
        'CONFLICT_VERSION_STALE',
        `base_version_id ${baseVersionId} is no longer current (current is ${doc.currentVersionId}).`,
        { expected: baseVersionId, actual: doc.currentVersionId },
        'retry_now',
      )
    }
    ...
    try {
      newContent = applyJsonPatch(baseVersion.content as Record<string, unknown>, delta)
    } catch (err) {
      throw new OrbitalError(
        'VALIDATION_INVALID_JSON_PATCH',
        `JSON Patch application failed: ${(err as Error).message}`,
        {},
        'no_retry',
      )
    }
    ...
    if (!validation.ready) {
      throw new OrbitalError(
        'VALIDATION_REQUIRED_FIELD_MISSING',
        `Revised content missing required fields: ${validation.missing_fields.join(', ')}`,
        { missing_fields: validation.missing_fields },
        'no_retry',
      )
    }
    ...
    const hookDecision = await this.hookEngine.fire(
      ...
      { trace_id: traceId, actor, capability_id: undefined },
      'pre',
    )
    ...
    if (!hookDecision.allow) {
      throw new OrbitalError(
        'HOOK_REJECTED_PRE_LOCK',
        `Revise rejected by hook: ${hookDecision.reason}`,
        { hook_id: hookDecision.hook_id },
        'escalate_to_human',
      )
    }

    const newVersionId = uuidv7() as VisionVersionId
    ...
    try {
      await this.db.insert(visionVersions).values({
        ...
        contentHash,
        changelog,
        deltaFromPrevious: delta as unknown as Record<string, unknown>[],
      ...
      if ((insertErr as { code?: unknown }).code === '23505') {
        throw new OrbitalError(
          'CONFLICT_OPTIMISTIC_LOCK_FAILED',
          `Concurrent revision detected: version_number ${newVersionNumber} for document ${documentId} was already claimed. Retry with the latest base_version_id.`,
          { vision_document_id: documentId, version_number: newVersionNumber },
          'retry_with_backoff',
        )
      }
      throw insertErr
    ...
    if (revisedRows.length === 0) {
      throw new OrbitalError(
        'CONFLICT_OPTIMISTIC_LOCK_FAILED',
        `Concurrent modification detected: vision document ${documentId} was modified during revise. Retry with the latest base_version_id.`,
        { document_id: documentId, expected_version: doc.currentVersionNumber },
        'retry_now',
      )
    }
    ...
    logger.info(
      { documentId, newVersionId, newVersionNumber, priorVersionId: baseVersionId },
      'vision_revised',
    )
  ...
  async getDocument(documentId: VisionDocumentId): Promise<VisionDocument> {
    ...
    return {
      ...
      title: row.title,
      lifecycle_state: row.lifecycleState as 'drafting' | 'locked' | 'revised' | 'abandoned',
  ...
  ): Promise<VisionDocumentVersion | null> {
      ...
      const rows = await this.db
        ...
        .where(
          and(
            ...
            eq(visionVersions.versionNumber, versionNumber),
          ),
        )
        .limit(1)
  ...
  async listMessages(
    ...
    return rows.map((row) => ({
      ...
      author_type: row.authorType as 'user' | 'pm_persona',
      body: row.body,
...
function removeNestedValue(obj: Record<string, unknown>, path: string[]): void {
  ...
  const key = path[0]!
  if (key in obj && typeof obj[key] === 'object') {
    removeNestedValue(obj[key] as Record<string, unknown>, path.slice(1))
  }
}
...
export function createVisionService(
  ...
  capabilityAuthority: ICapabilityAuthority,
