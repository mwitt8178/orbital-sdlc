...
// ---------------------------------------------------------------------------
// State union (Primitives §13)
// ---------------------------------------------------------------------------

export type VisionLifecycleState = 'drafting' | 'locked' | 'revised' | 'abandoned'

export type VisionLifecycle =
  | { state: 'drafting'; draft_version_id: string | null; open_session_id: string | null }
  | { state: 'locked'; current_version_id: string; locked_at: string }
  | { state: 'revised'; current_version_id: string; previous_locked_version_id: string; revised_at: string }
  | { state: 'abandoned'; abandoned_at: string; reason: string }
...
export function assertTransitionAllowed(
  currentState: VisionLifecycleState,
  targetState: VisionLifecycleState,
): void {
  const allowed = ALLOWED_TRANSITIONS[currentState] ?? new Set<VisionLifecycleState>()
  if (!allowed.has(targetState)) {
    throw new OrbitalError(
      'CONFLICT_INVALID_STATE_TRANSITION',
      `Vision document cannot transition from '${currentState}' to '${targetState}'`,
...
export function assertLockAllowed(
  ...
  if (currentState !== 'drafting') {
    throw new OrbitalError(
      'CONFLICT_INVALID_STATE_TRANSITION',
      `Lock attempted on a document in '${currentState}' state. Only 'drafting' documents can be locked.`,
  ...
  if (actorType !== 'user') {
    throw new OrbitalError(
      'CONFLICT_INVALID_STATE_TRANSITION',
      'FR-1.3: vision.lock requires actor.type === \'user\'. Only a human can lock a vision document.',
 ...
 */
export function assertReviseAllowed(
  currentState: VisionLifecycleState,
  actorType: string,
): void {
  if (currentState !== 'locked' && currentState !== 'revised') {
    throw new OrbitalError(
      'CONFLICT_INVALID_STATE_TRANSITION',
      `Revise attempted on a document in '${currentState}' state. Only 'locked' or 'revised' documents can be revised.`,
      { current_state: currentState },
      'no_retry',
    )
  }
  if (actorType !== 'user') {
    throw new OrbitalError(
      'CONFLICT_INVALID_STATE_TRANSITION',
      'vision.revise requires actor.type === \'user\'.',
      { actor_type: actorType },
      'no_retry',
    )
  }
...
// ---------------------------------------------------------------------------
// Lock validation helpers (FR-1.7)
// ---------------------------------------------------------------------------
...
export function validateForLock(
  content: Partial<VisionDocumentContent> | Record<string, unknown>,
  attestation: { no_edge_cases: boolean },
): LockValidationResult {
  const missing: string[] = []
  const blockingQs: string[] = []

  const c = content as Record<string, unknown>

  // Required non-empty arrays (FR-1.2, FR-1.7)
  for (const field of ['goals', 'non_goals', 'target_users', 'acceptance_criteria'] as const) {
    const arr = c[field]
  ...
  if (Array.isArray(open_questions)) {
    for (const q of open_questions as Array<Record<string, unknown>>) {
      if (q['blocking'] === true && q['resolved_at'] === undefined) {
        blockingQs.push(q['id'] as string)
      }
    }
  }
...
// In-process token store (keyed by token → {document_id, expires_at}).
// Per TRD-01 §8.4: 15-minute TTL, single-use.
// NOTE: In production this could be a Redis store; for single-process use the
// in-memory map is sufficient and matches the spec guidance.
...
export function issueConfirmationToken(documentId: string): string {
...
export function consumeConfirmationToken(
  ...
  if (!entry) {
    throw new OrbitalError(
      'CONFLICT_CONFIRMATION_TOKEN_USED',
      'Confirmation token not found or already consumed.',
      { token: '[redacted]' },
      'no_retry',
    )
  }
  ...
  if (Date.now() > entry.expiresAt) {
    ...
    throw new OrbitalError(
      'CONFLICT_CONFIRMATION_TOKEN_USED',
      'Confirmation token has expired (15-minute TTL).',
      {},
      'no_retry',
    )
  }
  if (entry.documentId !== documentId) {
    throw new OrbitalError(
      'CONFLICT_CONFIRMATION_TOKEN_USED',
      'Confirmation token was issued for a different document.',
      {},
      'no_retry',
    )
  }
...
const ALLOWED_TRANSITIONS: Record<VisionLifecycleState, Set<VisionLifecycleState>> = {
