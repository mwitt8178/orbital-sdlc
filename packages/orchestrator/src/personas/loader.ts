...
import {
  ...
  personaSkills,
  personaCapabilities,
  personaModelAffinities,
} from '../db/schema/personas.js'
import type { PersonaDefinition, Persona } from './types.js'
...
// ---------------------------------------------------------------------------
// PersonaLoader interface (per Implementation Plan §6 Task 2A exposes)
// ---------------------------------------------------------------------------
...
export class DefaultPersonaLoader implements PersonaLoader {
  constructor(
  ...
  async load(): Promise<void> {
    ...
    logger.info(
      { loaded, updated, skipped, failed, total: definitions.length },
      'personas_loaded',
    )
  }
  ...
  async get(personaId: string): Promise<Persona> {
    ...
    const row = rows[0]
    if (!row) throw new Error(`NOT_FOUND_PERSONA: ${personaId}`)
  ...
  async getActive(): Promise<Persona[]> {
    const rows = await this.db
      .select()
      .from(personas)
  ...
  private async collectDefinitions(): Promise<PersonaDefinition[]> {
    const defsMap = new Map<string, PersonaDefinition>()
  ...
  private async upsertPersona(def: PersonaDefinition): Promise<'created' | 'updated' | 'noop'> {
    ...
    if (existing) {
      ...
      await this.db.transaction(async (tx) => {
        ...
        await tx.insert(personaVersions).values({
          ...
          definitionHash: hash,
          escalationPolicy: def.escalationPolicy as unknown as Record<string, unknown>,
        ...
        await tx.insert(personaCapabilities).values({
          personaVersionId: versionId,
          defaultProfileJson: def.defaultCapabilityProfile as unknown as Record<string, unknown>,
        })
  ...
  private async nextVersionNumber(personaId: string): Promise<number> {
    const rows = await this.db
      .select()
      .from(personaVersions)
  ...
  private async insertSkillsForVersion(
  ...
  private async createStubSkillVersion(
    tx: Parameters<Parameters<DB['transaction']>[0]>[0],
    skillId: string,
    slug: string,
  ): Promise<string> {
  ...
  private async insertModelAffinitiesForVersion(
    tx: Parameters<Parameters<DB['transaction']>[0]>[0],
    versionId: string,
    def: PersonaDefinition,
  ): Promise<void> {
  ...
  private computeHash(def: PersonaDefinition): string {
    const normalized = JSON.stringify(def, Object.keys(def).sort())
    return createHash('sha256').update(normalized).digest('hex')
  }

  private computeStringHash(s: string): string {
  ...
  private async assemblePersona(personaId: string, versionId: string): Promise<Persona> {
    ...
    if (!version) throw new Error(`INTERNAL_DB_ERROR: persona_version ${versionId} not found`)
    ...
    if (!personaRow) throw new Error(`INTERNAL_DB_ERROR: persona ${personaId} not found`)

    const def = version.definitionJson as unknown as PersonaDefinition
    ...
    return {
      ...
      defaultCapabilityProfile: cap.defaultProfileJson as unknown as PersonaDefinition['defaultCapabilityProfile'],
      modelAffinity: affinityRows.map((a) => ({
        riskClass: a.riskClass as PersonaDefinition['modelAffinity'][number]['riskClass'],
        preferredModel: a.preferredModel as PersonaDefinition['modelAffinity'][number]['preferredModel'],
        fallbackModel: (a.fallbackModel ?? null) as PersonaDefinition['modelAffinity'][number]['fallbackModel'],
        maxTokensHint: a.maxTokensHint ?? null,
        rationale: a.rationale,
      })),
      escalationPolicy: version.escalationPolicy as unknown as PersonaDefinition['escalationPolicy'],
...
export function createPersonaLoader(db: DB, eventStore: EventStore): PersonaLoader {
