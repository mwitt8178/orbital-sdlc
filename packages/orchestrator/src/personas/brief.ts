...
import {
  injectMemoryIntoBrief,
  type MemoryBriefInjection,
} from '../memory/brief-injector.js'
import type { RoutingEngine } from '../routing/engine.js'

// ---------------------------------------------------------------------------
// Task shape — minimal interface for what the brief builder needs
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Optional brief extensions used by the AnthropicDriver call sites
// ---------------------------------------------------------------------------
...
explicitly with a NOTE: comment.`)
  ...
  if (extensions.conversationHistory && extensions.conversationHistory.messages.length > 0) {
    const lines = extensions.conversationHistory.messages.map(
      (m) => `**${m.author}:** ${m.body}`,
    )
 ...
 */
export function buildBriefSync(
  persona: Persona,
  task: BriefTask,
  capability: CapabilityBundle,
  extensions: Omit<BriefExtensions, 'memoryContext'> = {},
): string {
  ...
  const scopes = capability.scopes
  const scopeParts: string[] = []
  if (scopes.files_read?.length) scopeParts.push(`**Files you may READ:** ${scopes.files_read.join(', ')}`)
  if (scopes.files_write?.length) scopeParts.push(`**Files you may WRITE:** ${scopes.files_write.join(', ')}`)
  if (scopes.channel_post?.length) scopeParts.push(`**Channels you may POST to:** ${scopes.channel_post.join(', ')}`)
  if (scopes.channel_read?.length) scopeParts.push(`**Channels you may READ:** ${scopes.channel_read.join(', ')}`)
  if (scopes.board_mutate?.length) scopeParts.push(`**Board fields you may MUTATE:** ${scopes.board_mutate.join(', ')}`)
  if (scopes.secrets?.length) scopeParts.push(`**Secrets you may ACCESS:** ${scopes.secrets.join(', ')}`)
  if (scopes.network_egress?.length) scopeParts.push(`**Network egress allowed to:** ${scopes.network_egress.join(', ')}`)
  ...
  if (extensions.visionContext) {
    ...
    if (v.title) parts.push(`**Title:** ${v.title}`)
    if (v.summary) parts.push(`**Summary:** ${v.summary}`)
    if (v.goals?.length) parts.push(`**Goals:**\n${v.goals.map((g) => `- ${g}`).join('\n')}`)
    if (v.nonGoals?.length) parts.push(`**Non-goals:**\n${v.nonGoals.map((g) => `- ${g}`).join('\n')}`)
    if (v.existingEpicTitles?.length) parts.push(`**Existing epics:**\n${v.existingEpicTitles.map((t) => `- ${t}`).join('\n')}`)
    if (parts.length > 0) sections.push(`# Vision Context\n\n${parts.join('\n\n')}`)
  }
