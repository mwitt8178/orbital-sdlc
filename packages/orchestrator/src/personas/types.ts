...
export const DefaultCapabilityProfileSchema = z.object({
  ...
  secrets: z.array(z.string()).refine(
    (arr) => arr.every((s) => !s.includes('*')),
    { message: 'secrets scope must not contain wildcards' },
  ),
...
export const EscalationRuleSchema = z
  ...
  .refine(
    (r) => r.action !== 'spawn_resolver' || typeof r.resolverPersona === 'string',
    { message: 'spawn_resolver action requires resolverPersona' },
  )
...
export const PersonaDefinitionSchema = z
  .object({
    ...
    modelAffinity: z
      ...
      .refine(
        (arr) => arr.some((e) => e.riskClass === 'standard'),
        { message: 'modelAffinity must include an entry for riskClass="standard"' },
      ),
  ...
  .refine(
    (d) => d.roleBrief.bodyMd.length <= 32768,
    { message: 'roleBrief.bodyMd must not exceed 32 KB' },
  )

export type PersonaDefinition = z.infer<typeof PersonaDefinitionSchema>

// ---------------------------------------------------------------------------
// Persona — the DB-persisted shape returned to callers
// ---------------------------------------------------------------------------
