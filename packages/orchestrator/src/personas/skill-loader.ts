...
import type { Persona, SkillRef } from './types.js'
import { BASELINE_PERSONAS } from './library/index.js'
import type { EventStore } from '../events/store.js'
import type { SkillLoadedPayload } from '../events/types.js'
 ...
 * (node, dist/) modes.
 */
function getSkillsSearchRoots(): string[] {
  const here = fileURLToPath(import.meta.url)
  const dir = path.dirname(here)
 ...
 */
export async function bundleSkillsForWorker(
  persona: Persona,
  worktreePath: string,
  options: BundleSkillsOptions = {},
): Promise<SkillsBundleResult> {
    ...
    try {
      ...
      if (options.eventStore) {
        ...
        await options.eventStore.append({
          ...
          payload: skillPayload as unknown as Record<string, unknown>,
          actor: SYSTEM_ACTOR,
          trace_id: uuidv7(),
      ...
      logger.warn(
        { err, slug: ref.slug, source: resolved, target: targetPath },
        'skill-loader: failed to copy skill file',
      )
      missing.push(ref.slug)
    }
  }
  ...
  if (missing.length > 0) {
    logger.warn(
      { personaSlug: persona.slug, missingSkills: missing },
      'skill-loader: persona references skills with no markdown file',
    )
  }
...
async function resolveSkillFile(
  ...
  for (const root of roots) {
    ...
    if (alias) {
      ...
      if (await exists(aliased)) return aliased
    }
  }
  return null
}
 ...
 * skills to copy.
 */
function resolveSkillRefs(persona: Persona): SkillRef[] {
  const allUnknown =
    persona.skills.length > 0 && persona.skills.every((s) => s.slug === 'unknown')
  if (!allUnknown) return persona.skills

  const baseline = BASELINE_PERSONAS.find((p) => p.slug === persona.slug)
  if (!baseline) return persona.skills
...
export function _getSkillsRootForTests(): string {
  return getSkillsRoot()
}

/** Test-only: return a snapshot of the alias map. */
export function _getSkillAliasesForTests(): Record<string, string> {
