...
export function buildNLParserSystemPrompt(): string {
  return STATIC_SYSTEM
}

export function buildNLParserUserPrompt(prompt: string, ctx: NLParserContext): string {
  const lines: string[] = []
  lines.push('## Vision context')
  if (ctx.visionTitle) lines.push(`Title: ${ctx.visionTitle}`)
  if (ctx.visionSummary) lines.push(`Summary: ${ctx.visionSummary}`)
  if (ctx.topGoals.length > 0) {
    lines.push('Goals:')
    for (const g of ctx.topGoals) lines.push(`  - ${g}`)
  }
  ...
  if (ctx.existingEpicTitles.length === 0) {
    ...
    for (const t of ctx.existingEpicTitles) lines.push(`- ${t}`)
  }
