...
export function buildPersonaOfRecordSystemPrompt() {
    return STATIC_SYSTEM;
}
export function buildPersonaOfRecordUserPrompt(ctx) {
    const lines = [];
    lines.push(`## Defect`);
    ...
    for (const slug of ctx.candidatePersonaSlugs)
        lines.push(`- ${slug}`);
    lines.push('');
    lines.push(`## Fallback if you have insufficient signal`);
    lines.push(`tasks.persona_id resolves to "${ctx.tasksFallbackPersonaId}". Return that ` +
