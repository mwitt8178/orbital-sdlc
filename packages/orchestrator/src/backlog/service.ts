...
export interface BacklogService {
  ...
  // Estimation (Scrum Master persona writes points + rationale)
  estimateStory(
    storyId: string,
    storyPoints: number,
    rationale: string,
  ...
  // Groom: bulk refine + estimate (TRD-02 §6.1 backlog.refine; lightweight server-side
  //        materialization that records grooming decisions)
  groom(input: {
    storyId: string
...
export class DefaultBacklogService implements BacklogService {
  constructor(
  ...
  async createEpic(input: CreateEpicInput, actor: Actor = SYSTEM_ACTOR, tenantId: string = SENTINEL_TENANT): Promise<EpicRow> {
  ...
  async listEpics(filter: {
    status?: string
    visionVersionId?: string
  } = {}, tenantId: string = SENTINEL_TENANT): Promise<EpicRow[]> {
    const conditions: ReturnType<typeof eq>[] = [eq(epics.tenantId, tenantId)]
    if (filter.status) conditions.push(eq(epics.status, filter.status as 'draft'))
  ...
  async getEpic(epicId: string, tenantId: string = SENTINEL_TENANT): Promise<EpicRow | null> {
  ...
  async createStory(
  ...
  ): Promise<StoryWithAcceptanceCriteria> {
    ...
    if (!epic) {
      throw new OrbitalError(
        BACKLOG_ERROR_CODES.NOT_FOUND_EPIC,
        `epic ${parsed.epic_id} not found`,
      )
    }
    ...
    const lastStories = await this.db
      ...
      .limit(1)
    const nextPriority =
      parsed.priority ?? ((lastStories[0]?.priority ?? -1) + 1)
  ...
  async listStories(
    filter: { epicId?: string; status?: StoryStatus; sprintId?: string } = {},
    tenantId: string = SENTINEL_TENANT,
  ): Promise<StoryRow[]> {
    const conditions: ReturnType<typeof eq>[] = [eq(stories.tenantId, tenantId)]
    if (filter.epicId) conditions.push(eq(stories.epicId, filter.epicId))
    if (filter.status) conditions.push(eq(stories.status, filter.status))
    const where = and(...conditions)
  ...
  async getStory(storyId: string, tenantId: string = SENTINEL_TENANT): Promise<StoryWithAcceptanceCriteria | null> {
  ...
  private async getStoryRequired(storyId: string, tenantId: string = SENTINEL_TENANT): Promise<StoryWithAcceptanceCriteria> {
    const story = await this.getStory(storyId, tenantId)
    if (!story) {
      throw new OrbitalError(
        BACKLOG_ERROR_CODES.NOT_FOUND_STORY,
  ...
  async updateStory(
    ...
    const result = await this.db.transaction(async (tx) => {
      await tx.execute(
        dSQL`SELECT pg_advisory_xact_lock(hashtext(${'story:' + parsed.story_id}))`,
      )
      ...
      if (!current) {
        throw new OrbitalError(
          BACKLOG_ERROR_CODES.NOT_FOUND_STORY,
          `story ${parsed.story_id} not found`,
        )
      }

      // Status transition validation
      let willEmitStatusChange = false
      let fromStatus: StoryStatus = current.status
      let toStatus: StoryStatus = current.status
      if (parsed.status && parsed.status !== current.status) {
        if (!isValidStoryTransition(current.status, parsed.status)) {
          throw new OrbitalError(
            ...
            `invalid story transition: ${current.status} -> ${parsed.status}`,
            { from: current.status, to: parsed.status },
          )
        }

        // FR-2.6 enforcement: linked artifacts required for certain transitions
        const transitionRequires = this.linkedArtifactRequirement(
          current.status,
          parsed.status,
        )
        if (transitionRequires) {
          ...
          if (!ok) {
            throw new OrbitalError(
              BACKLOG_ERROR_CODES.VALIDATION_LINKED_ARTIFACT_MISSING,
              `transition ${current.status}->${parsed.status} requires linked_artifacts of type [${transitionRequires.join(',')}]`,
              { required_types: transitionRequires },
            )
          }
        }
      ...
      if (parsed.story_points !== undefined) patch.storyPoints = parsed.story_points
      if (parsed.persona_of_record !== undefined) patch.personaOfRecord = parsed.persona_of_record
  ...
  async movStoryToPosition(
    ...
    if (oldPriority === position) return

    const changes: Array<{ aggregate_id: string; old_priority: number; new_priority: number }> = []

    await this.db.transaction(async (tx) => {
      // Lock the epic-level row range
      await tx.execute(
        dSQL`SELECT pg_advisory_xact_lock(hashtext(${'epic:' + story.epicId}))`,
      )
      ...
      if (oldPriority < position) {
        ...
        const affected = await tx
          ...
          .where(
            and(
              eq(stories.epicId, story.epicId),
              gte(stories.priority, oldPriority + 1),
              lte(stories.priority, position),
            ),
          )
        ...
        const affected = await tx
          ...
          .where(
            and(
              eq(stories.epicId, story.epicId),
              gte(stories.priority, position),
              lte(stories.priority, oldPriority - 1),
            ),
          )
        for (const r of affected) {
          changes.push({
  ...
  async estimateStory(
    ...
    if (!Number.isInteger(storyPoints) || storyPoints <= 0) {
      throw new OrbitalError(
        BACKLOG_ERROR_CODES.VALIDATION_REQUIRED_FIELD_MISSING,
        'story_points must be a positive integer',
      )
    }
    if (!rationale.trim()) {
      throw new OrbitalError(
  ...
  async groom(
    ...
    if (!story) {
      throw new OrbitalError(
        BACKLOG_ERROR_CODES.NOT_FOUND_STORY,
        `story ${input.storyId} not found`,
      )
    }

    const acAdded: string[] = []
    const acRemoved: string[] = input.removeAcIds ?? []

    await this.db.transaction(async (tx) => {
      await tx.execute(
        dSQL`SELECT pg_advisory_xact_lock(hashtext(${'story:' + input.storyId}))`,
      )
      ...
      if (acRemoved.length > 0) {
        await tx
          ...
          .where(
            and(
              eq(storyAcceptanceCriteria.storyId, input.storyId),
              inArray(storyAcceptanceCriteria.acId, acRemoved),
            ),
          )
      }
      ...
      if (input.addAcs && input.addAcs.length > 0) {
        ...
        for (const r of newRows) acAdded.push(r.acId)
      }
...
export function createBacklogService(db: DB, eventStore: EventStore): BacklogService {
