...
import { eq, and, lt, lte, gt, gte, desc, asc, or, type SQL } from 'drizzle-orm'
...
// ---------------------------------------------------------------------------
// EventStore interface (exported per Task 1A spec)
// ---------------------------------------------------------------------------
...
export class PostgresEventStore implements EventStore {
  ...
  constructor(
    ...
    this.notifyClient = new NotifyClient(
      sql,
      (eventId) => this.getById(eventId),
      (afterEventId) => this.backfillSince(afterEventId),
    )
  }
  ...
  async append(event: EventInput): Promise<EventEnvelope> {
  ...
  async query(filter: EventQueryFilter): Promise<PaginatedResponse<EventEnvelope>> {
    ...
    if (filter.actor_id !== undefined) {
      ...
      if (filter.actor_type === 'persona') {
        conditions.push(
          dSQL`${events.actor}->>'persona_id' = ${filter.actor_id}`,
        )
      } else if (filter.actor_type === 'user') {
        conditions.push(
          dSQL`${events.actor}->>'user_id' = ${filter.actor_id}`,
        )
      } else if (filter.actor_type === 'hook') {
        conditions.push(
          dSQL`${events.actor}->>'hook_id' = ${filter.actor_id}`,
        )
      } else {
      ...
      conditions.push(
        dSQL`${events.actor}->>'type' = ${filter.actor_type}`,
      )
    }
  ...
  subscribe(cursor: string | null, handler: (event: EventEnvelope) => void): () => void {
    ...
    void this.notifyClient.start(cursor).catch((err: unknown) => {
   ...
   * Per TRD-07 §4.1.4.
   */
  private async backfillSince(afterEventId: string | null): Promise<EventEnvelope[]> {
  ...
  async stopNotifyClient(): Promise<void> {
 ...
 */
export function createEventStore(db: DB, sql: postgres.Sql): EventStore {
 ...
 */
export function sanitizeEventForHubFanout(envelope: EventEnvelope): { ok: true } | { ok: false; error: LocalDataLeakError } {
  try {
    ...
    if (err instanceof LocalDataLeakError) {
      logger.fatal(
        { event_id: envelope.event_id, event_type: envelope.event_type, path: err.path, reason: err.reason },
        'CRITICAL: blocked event from hub fanout — local-only data detected',
      )
...
// ---------------------------------------------------------------------------
// Round 7-06 — Outbox fallback helper
// [Engineer-Sr · Sonnet · run-round7-06-offline-reconcile]
//
// appendWithOutboxFallback() is the recommended call path for the MCP audit
// middleware and any caller that needs:
//   1. Local write always succeeds (even hub offline).
//   2. Hub-bound mirror goes through the persistent outbox when hub is down.
//
// Usage:
//   import { appendWithOutboxFallback } from '../events/store.js'
//   await appendWithOutboxFallback(store, eventInput, hubClient, outbox)
//
// When hub is down: local write succeeds + outbox row created.
// When hub is up: local write succeeds + direct hub fanout (no outbox row).
// Idempotency: the hub call includes the event_id as the idempotency key.
// ---------------------------------------------------------------------------

import type { HubClient } from '../hub-client/client.js'
 ...
 */
export async function appendWithOutboxFallback(
  store: PostgresEventStore,
  input: EventInput,
  hubClient: HubClient | null,
  outbox: HubOutbox | null,
): Promise<EventEnvelope> {
  ...
  const hubEventInput = {
    ...
    event_type: envelope.event_type,
    payload: envelope.payload,
    actor: envelope.actor as Record<string, unknown>,
    capability_id: envelope.capability_id,
    trace_id: envelope.trace_id,
    occurred_at: envelope.occurred_at,
    schema_version: envelope.schema_version,
    tenant_id: (envelope.payload as Record<string, unknown>)['tenant_id'] as string ?? '',
  }
  ...
  if (hubClient.connectionState === 'offline') {
    ...
    logger.debug(
      { event_id: envelope.event_id, event_type: envelope.event_type },
      'events/store: hub offline — enqueued to outbox',
    )
  } else {
    ...
    if (!result.ok) {
      ...
      logger.warn(
        { event_id: envelope.event_id, event_type: envelope.event_type },
        'events/store: hub fanout failed — enqueued to outbox',
      )
    }
  }
