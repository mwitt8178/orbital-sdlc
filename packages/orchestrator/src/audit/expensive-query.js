...
export function isExpensiveQuery(filter) {
    ...
    if (hasAggregateId || hasEventType)
        return false;
 ...
 */
export function expensiveQueryReason(filter) {
    if (!isExpensiveQuery(filter))
        return null;
    const reasons = [];
    if (!filter.aggregate_id)
        reasons.push('no aggregate_id filter');
...
//# sourceMappingURL=expensive-query.js.map