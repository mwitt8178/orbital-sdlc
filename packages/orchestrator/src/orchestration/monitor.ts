    ...
    for (const w of candidates) {
      ...
      if (child && !child.killed && child.exitCode === null) {
        try {
          ...
          logger.warn(
            { err, workerId: w.workerId },
            'WorkerMonitor: SIGTERM failed; will SIGKILL after grace',
          )
        }
        ...
        setTimeout(() => {
          void (async () => {
            ...
            logger.error(
              { err, workerId },
              'WorkerMonitor: failed to finalize worker termination',
            )
          })
...
export function createWorkerMonitor(
  db: DB,
