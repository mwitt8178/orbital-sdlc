...
export interface IPauseController {
   ...
   */
  pauseDueToCostCeiling(
    scope: 'sprint' | 'project' | 'install',
    scopeId: string,
    reason: string,
  ): Promise<PauseResult>
}
...
export class PauseController implements IPauseController {
  ...
  async pause(sprintId: string, traceId: string): Promise<PauseResult> {
    ...
    for (const w of finalWorkers) {
      try {
        ...
        logger.warn(
          { err, workerId: w.workerId, capabilityId: w.capabilityId },
          'PauseController: revoke failed (likely already revoked); continuing',
        )
      }
    ...
    logger.info(
      { sprintId, drained: drainedWorkerIds.length, revoked: revokedCapabilityIds.length },
      'PauseController: paused sprint',
    )
  ...
  async resume(sprintId: string, traceId: string): Promise<ResumeResult> {
    ...
    for (const t of readyTasks) {
      try {
        ...
        logger.warn(
          { err, taskId: t.taskId },
          'PauseController.resume: failed to re-issue capability for task; continuing',
        )
      }
    }
    ...
    logger.info(
      { sprintId, reissued: reissuedCapabilityCount, resumedTasks: resumedTaskIds.length },
      'PauseController: resumed sprint',
    )
  ...
  async pauseDueToCostCeiling(
    ...
    logger.warn(
      ...
      'PauseController.pauseDueToCostCeiling: pausing scope due to budget ceiling',
    )
  ...
  private async findActiveSprintWorkers(sprintId: string): Promise<
    Array<{
      workerId: string
      taskId: string | null
      capabilityId: string
   ...
   */
  private async signalWorkers(
    workers: Array<{ workerId: string; pid: number | null; status: string }>,
  ): Promise<void> {
    ...
    for (const w of workers) {
      if (!signalableStatuses.has(w.status)) {
        logger.debug(
          ...
          'PauseController.signalWorkers: skipping worker in non-signalable status',
        )
        continue
      }
      if (w.pid == null) {
        logger.warn(
          { workerId: w.workerId },
          'PauseController.signalWorkers: no pid recorded for worker; cannot SIGTERM',
        )
        continue
      }
      try {
        ...
        logger.info(
          { workerId: w.workerId, pid: w.pid },
          'PauseController.signalWorkers: SIGTERM sent',
        )
        ...
        if (code === 'ESRCH') {
          logger.debug(
            { workerId: w.workerId, pid: w.pid },
            'PauseController.signalWorkers: process already exited before SIGTERM',
          )
        } else {
          logger.warn(
            { err, workerId: w.workerId, pid: w.pid },
            'PauseController.signalWorkers: SIGTERM failed',
          )
        }
      }
    }

    if (pidsToKill.length === 0 || this.sigkillGraceMs <= 0) return
    ...
    for (const pid of pidsToKill) {
      try {
        ...
        logger.warn(
          { pid },
          'PauseController.signalWorkers: process survived SIGTERM grace; SIGKILL sent',
        )
      } catch (err) {
        const code = (err as NodeJS.ErrnoException).code
        if (code === 'ESRCH') {
          logger.debug(
            { pid },
            'PauseController.signalWorkers: process exited cleanly after SIGTERM',
          )
        } else {
          logger.warn(
            { err, pid },
            'PauseController.signalWorkers: SIGKILL check failed',
          )
        }
      }
    }
  ...
  private async flipPaused(paused: boolean, sprintId: string): Promise<void> {
    const rows = await this.db.select().from(workerPoolState).limit(1)
  ...
  private async emitResumeApplied(
    sprintId: string,
    resumedTaskIds: string[],
    reissuedCapabilityCount: number,
    traceId: string,
