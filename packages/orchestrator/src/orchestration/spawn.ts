...
import { spawn as childSpawn, spawnSync, type ChildProcess } from 'node:child_process'
import { promises as fs } from 'node:fs'
import path from 'node:path'
import { eq } from 'drizzle-orm'
import { OrbitalError, type CapabilityBundle, type Actor, type EventInput } from '@orbital/types'
...
export async function spawn(
  ...
  if (params.realClaude) {
    if (!env.ANTHROPIC_API_KEY) {
      throw new OrbitalError(
        ORCHESTRATION_ERROR_CODES.AUTH_INVALID_CAPABILITY,
        'STARTUP_ERROR: ANTHROPIC_API_KEY is required to spawn a real Claude Code worker. ' +
          'Set ANTHROPIC_API_KEY in your env or use the test surrogate path.',
  ...
  let copiedSkills: string[] | undefined
  if (params.realClaude) {
    ...
    const bundleResult = await bundleSkillsForWorker(
      params.realClaude.persona,
      params.worktreePath,
    )
  ...
  if (!params.reuseWorktree) {
    try {
      ...
      if (branchResult.status !== 0) {
        ...
        logger.warn(
          { taskId: params.taskId, branchName: agentBranchName, stderr: stderr.slice(0, 200) },
          'spawn: git checkout -B for agent branch failed; worker will run on current branch',
        )
      } else {
        logger.debug(
          { taskId: params.taskId, branchName: agentBranchName },
          'spawn: agent branch created',
        )
      }
    } catch (err) {
      logger.warn(
        { err, taskId: params.taskId, branchName: agentBranchName },
        'spawn: git branch creation threw; continuing without branch switch',
      )
    }
  } else {
    logger.debug(
  ...
  try {
    ...
    if (isENOENT(err)) {
      ...
      throw new OrbitalError(
        ORCHESTRATION_ERROR_CODES.CLAUDE_BIN_NOT_FOUND,
        formatNotFoundMessage(claudeBin),
        { claude_bin: claudeBin, code: 'ENOENT' },
      )
    }
    await fs.unlink(capabilityFilePath).catch(() => undefined)
    throw err
  ...
  if (earlyError) {
    ...
    throw new OrbitalError(
      ORCHESTRATION_ERROR_CODES.INTERNAL_SPAWN_ABORTED,
      `child process spawn failed: ${(earlyError as Error).message}`,
  ...
  let outputStream: WorkerOutputStream | null = null
  try {
    ...
    logger.warn(
      { err, workerId, taskId: params.taskId },
      'spawn: failed to attach WorkerOutputStream; falling back to debug-only logging',
    )
    outputStream = null
    // Fall back to the original debug-level log piping.
    child.stdout?.on('data', (chunk: Buffer) => {
      logger.debug(
        { taskId: params.taskId, workerId, stream: 'stdout', text: chunk.toString().trimEnd() },
        'worker stdout',
      )
    })
    child.stderr?.on('data', (chunk: Buffer) => {
      logger.debug(
        { taskId: params.taskId, workerId, stream: 'stderr', text: chunk.toString().trimEnd() },
        'worker stderr',
      )
    })
  }
  ...
  if (hubClient !== null) {
    ...
    if (!hubReg.ok) {
      logger.warn(
        { workerId, hubError: hubReg.message },
        'spawn: hub worker registration failed (non-fatal; local write succeeded)',
      )
    }
  }
 ...
 */
function buildRealClaudeArgs(
  params: SpawnParams,
  ctx: RealClaudeContext,
  workerId: string,
): string[] {
 ...
 */
function buildPromptBody(task: BriefTask): string {
 ...
 */
function resolveModelArg(model: string): string {
  return model
}
...
function formatNotFoundMessage(claudeBin: string): string {
  return (
    `INTEGRATION_CLAUDE_NOT_FOUND: Claude CLI binary not found: ${claudeBin}. ` +
 ...
 */
function waitForEarlySpawnSignal(child: ChildProcess): Promise<Error | null> {
