...
export type {
  ...
  AgentWorkerRow,
  WorkerHeartbeatRow,
} from '../db/schema/orchestration.js'
...
// ---------------------------------------------------------------------------
// Sprint shape consumed by Scheduler.addSprint
// ---------------------------------------------------------------------------
...
export function isNonRetryable(errorCode: string): boolean {
  if (NON_RETRYABLE_CODES.has(errorCode)) return true
  if (errorCode.startsWith('INTERNAL_DATA_CORRUPTION_')) return true
  return false
}
