...
async function generateIamToken() {
    ...
    if (!region) {
        throw new Error('AWS_REGION is not set. Required for IAM DB auth (ORBITAL_DEPLOY_TARGET=aws).');
    }
    const hostname = e.RDS_PROXY_HOSTNAME;
    if (!hostname) {
        throw new Error('RDS_PROXY_HOSTNAME is not set. Required for IAM DB auth (ORBITAL_DEPLOY_TARGET=aws).');
    }
...
export const sql = new Proxy({}, {
    get(_t, prop) {
        if (!_sql) {
            throw new Error('sql is not initialised in AWS mode. Call await getDb() before accessing sql.');
        }
        return _sql[prop];
    },
});
...
export const db = new Proxy({}, {
    get(_t, prop) {
        if (!_db) {
            throw new Error('db is not initialised in AWS mode. Call await getDb() before accessing db.');
        }
        return _db[prop];
    },
});
