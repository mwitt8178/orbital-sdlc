...
export const UAT_AC_STATUS = ['pending', 'pass', 'fail'];
export const DEFECT_SEVERITY = ['critical', 'high', 'medium', 'low'];
 ...
 */
export const uatAcResults = pgTable('uat_ac_results', {
...
export const defectLineage = pgTable('defect_lineage', {
    ...
    ticketId: uuid('ticket_id').notNull(),
    taskIds: jsonb('task_ids').$type().notNull(),
    workerSessionIds: jsonb('worker_session_ids').$type().notNull(),
    primaryAuditEventIds: jsonb('primary_audit_event_ids').$type().notNull(),
 ...
 */
export const personaOfRecordLinks = pgTable('persona_of_record_links', {
