...
interface InstallKeyFile {
  /** UUIDv7. Stable for the lifetime of the install. */
  install_id: string
  /** ISO timestamp of generation. */
  created_at: string
  /** base64url Ed25519 public key (32 bytes). */
  public_key: string
  /** base64url Ed25519 private key (32 bytes). NEVER logged. */
  private_key: string
}
...
export function defaultInstallKeyPath(): string {
  const override = process.env['ORBITAL_INSTALL_KEY_PATH']
  if (override && override.length > 0) return override

  const orbitalHome =
    process.env['ORBITAL_HOME'] ?? path.join(os.homedir(), '.orbital')
 ...
 */
export async function loadInstallKey(filePath?: string): Promise<InstallKey | null> {
  ...
  let parsed: InstallKeyFile
  try {
    parsed = JSON.parse(raw) as InstallKeyFile
  } catch (err) {
    throw new Error(
      `install-key: ${target} is corrupt (JSON parse failed: ${(err as Error).message}). ` +
  ...
  if (
    ...
    throw new Error(
      `install-key: ${target} is missing required fields. ` +
...
export async function generateAndPersistInstallKey(opts: {
  ...
  const fileShape: InstallKeyFile = {
  ...
  logger.info(
    { installId, fingerprint, path: target },
    'install-key: generated new Ed25519 keypair',
  )
 ...
 */
export async function _purgeInstallKey(filePath?: string): Promise<void> {
