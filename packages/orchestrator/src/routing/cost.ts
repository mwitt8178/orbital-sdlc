...
// ---------------------------------------------------------------------------
// Rate table — micros per million tokens
// ---------------------------------------------------------------------------
...
const MODEL_RATES: Record<ModelId, ModelRates> = {
  'claude-opus-4-6': {
 ...
 */
export function computeCostMicros(model: ModelId, usage: AnthropicUsage): number {
  ...
  const M = 1_000_000
  const cost =
    (usage.input_tokens / M) * rates.input +
...
export class PostgresCostAccounting implements CostAccounting {
  constructor(
    private readonly db: DB,
    private readonly eventStore: EventStore,
  ) {}

  async report(params: CostReportParams): Promise<CostReportResult> {
    const {
      taskId,
      sessionId,
      sprintId,
    ...
    const existing = await this.db
      ...
      .where(
        and(
          ...
          eq(costAccounting.turnIndex, turnIndex),
        ),
      )
      .limit(1)
    ...
    // Check budget caps
    const budgetState = await this.checkAndUpdateBudgetCaps(
      taskId,
      sprintId,
      costUsdMicros,
      costId,
  ...
  async getSprintTotal(sprintId: SprintId): Promise<CostSummary> {
  ...
  async getTaskTotal(taskId: TaskId): Promise<CostSummary> {
    const rows = await this.db
      .select()
      .from(costAccounting)
  ...
  private async checkAndUpdateBudgetCaps(
    taskId: TaskId,
    sprintId: SprintId,
    newCostMicros: number,
    triggeringCostId: string,
    ...
    const sprintCapRows = await this.db
      ...
      .where(
        and(
          eq(budgetCaps.scope, 'sprint'),
          eq(budgetCaps.scopeKey, sprintId),
        ),
      )
      .limit(1)
    ...
    if (sprintCap && sprintCap.state !== 'overridden') {
      ...
      if (pct >= 100 && sprintCap.state !== 'exceeded') {
        ...
        await this.emitBudgetExceeded(
          capId, 'sprint', sprintId, sprintCap.capUsdMicros, consumed,
          triggeringCostId, triggeringSessionId, traceId,
        )
      ...
      if (pct >= sprintCap.warningThresholdPct && sprintCap.state === 'active') {
        ...
        await this.emitBudgetWarning(
          capId, 'sprint', sprintId, sprintCap.capUsdMicros, consumed, pct,
          sprintCap.warningThresholdPct, traceId,
        )

        return 'warned'
      }

      if (sprintCap.state === 'exceeded') return 'exceeded'
      if (sprintCap.state === 'warned') return 'warned'
    }
...
export function createCostAccounting(db: DB, eventStore: EventStore): CostAccounting {
