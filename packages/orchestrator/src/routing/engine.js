 ...
 */
export function decide(input, policy, catalog, policyVersion) {
    const rulesApplied = [];
    // Step 1: base model from persona affinity
    const personaAffinity = policy.persona_affinities.find((a) => a.persona_id === input.persona_id);
    let candidate = personaAffinity?.base_model ?? 'claude-sonnet-4-6';
    const baseFromPersona = candidate;
    if (!personaAffinity) {
        rulesApplied.push({
    ...
    if (input.latency_budget_ms != null) {
        ...
        if (catalogEntry && input.latency_budget_ms < catalogEntry.latency_p50_ms) {
            ...
            const sortedLatencyRules = [...policy.latency_rules].sort((a, b) => a.if_below_ms - b.if_below_ms);
            const latencyRule = sortedLatencyRules.find((r) => input.latency_budget_ms != null && input.latency_budget_ms < r.if_below_ms);
            if (latencyRule) {
    ...
    const tokenBudget = riskClassRule?.default_token_budget ??
...
// Tier helpers
// ---------------------------------------------------------------------------
function bumpTiers(model, tiers) {
...
export class DefaultRoutingEngine {
    db;
    eventStore;
    policy;
    catalog;
    policyVersion;
    constructor(db, eventStore, policy, catalog, policyVersion) {
        this.db = db;
    ...
    async selectModel(input) {
        ...
        const decision = { decision_id: decisionId, ...decisionCore };
        ...
        await this.db.transaction(async (tx) => {
            await tx.insert(routingDecisions).values({
                ...
                escalationPolicy: decision.escalation_policy,
    ...
    async routeModel(input) {
        const key = `${input.persona}:${input.estimate}`;
        const candidate = DEFAULT_ROUTING[key] ??
...
// Factory
// ---------------------------------------------------------------------------
export function createRoutingEngine(db, eventStore, policy, catalog, policyVersion) {
