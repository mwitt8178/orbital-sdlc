...
import { uuid, text, jsonb, integer, index, uniqueIndex } from 'drizzle-orm/pg-core'
import { sql } from 'drizzle-orm'
// timestamptz is accessed via a helper from drizzle-orm/pg-core — alias it clearly.
import { timestamp } from 'drizzle-orm/pg-core'
// audit schema object is declared in audit.ts — import here (not re-exported) to avoid
// duplicate export in the @orbital/db barrel. No callers import `audit` from events.ts.
...
export const events = audit.table(
  ...
  (t) => ({
    ...
    capabilityOccurredIdx: index('events_capability_occurred_at_idx').on(
      t.capabilityId,
      t.occurredAt,
    ),
