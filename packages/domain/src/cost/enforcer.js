...
import { agentWorkers } from '@orbital/db';
...
export class CostEnforcer {
    ...
    costService;
    killFn;
    constructor(db, eventStore, costService, 
    ...
    async canSpawn(params) {
        ...
        if (sprintId) {
            ...
            if (budget) {
                ...
                if (!result.allow) {
                    logger.warn({ projectId, sprintId, taskId, runningCostUsd, estimatedCostUsd, hardCap: budget.hardCapUsd }, 'CostEnforcer.canSpawn: sprint budget exceeded — blocking spawn');
                    await this._emitBudgetExceeded('sprint', sprintId, runningCostUsd, budget.hardCapUsd, uuidv7());
                    return result;
                }
                if (result.warn) {
                    logger.warn({ projectId, sprintId, runningCostUsd, hardCap: budget.hardCapUsd }, 'CostEnforcer.canSpawn: sprint budget soft threshold crossed');
                }
                return result;
            }
        }
        ...
        if (projectBudget) {
            ...
            if (!result.allow) {
                logger.warn({ projectId, runningCostUsd, estimatedCostUsd, hardCap: projectBudget.hardCapUsd }, 'CostEnforcer.canSpawn: project budget exceeded — blocking spawn');
                await this._emitBudgetExceeded('project', projectId, runningCostUsd, projectBudget.hardCapUsd, uuidv7());
                return result;
            }
            if (result.warn) {
                logger.warn({ projectId, runningCostUsd, hardCap: projectBudget.hardCapUsd }, 'CostEnforcer.canSpawn: project budget soft threshold crossed');
            }
            return result;
        }
     ...
     */
    async checkLive(projectId, sprintId) {
        ...
        if (!budget || budget.onHard !== 'kill')
            return;
        const runningCostUsd = await this.costService.getRunningCost(scope, scopeId);
        if (runningCostUsd <= budget.hardCapUsd)
            return;
        logger.error({ scope, scopeId, runningCostUsd, hardCap: budget.hardCapUsd }, 'CostEnforcer.checkLive: hard cap exceeded with kill policy — SIGTERMing all workers');
     ...
     */
    async killAll(scope, scopeId, reason, actorId) {
        const traceId = uuidv7();
    ...
    // Private
    // ---------------------------------------------------------------------------
    _evaluate(runningCostUsd, estimatedCostUsd, hardCapUsd, softThresholdPct) {
    ...
    async _emitBudgetExceeded(scope, scopeId, runningCostUsd, hardCapUsd, traceId) {
        const payload = {
            scope,
            scope_id: scopeId,
    ...
    async _killAllInScope(scope, scopeId, reason, traceId, actorId) {
        // Find all non-terminated workers in scope.
        let workerRows;
        if (scope === 'sprint') {
            const raw = await this.db.execute(dSQL `
        ...
        `);
            ...
            const raw = await this.db
                .select()
                .from(agentWorkers)
                .where(and(eq(agentWorkers.status, 'active')));
        ...
        for (const w of workerRows) {
            ...
            if (w.pid != null) {
                try {
                    ...
                    signalsSent++;
                    logger.info({ workerId: w.worker_id, pid: w.pid, reason, scope, scopeId }, 'CostEnforcer._killAllInScope: SIGTERM sent');
                }
                catch (err) {
                    const code = err.code;
                    if (code !== 'ESRCH') {
...
// Singleton registry
// ---------------------------------------------------------------------------
let _costEnforcer = null;
export function registerCostEnforcer(enforcer) {
    _costEnforcer = enforcer;
}
export function getCostEnforcer() {
    if (!_costEnforcer) {
        throw new Error('STARTUP_ERROR: CostEnforcer not registered. Call registerCostEnforcer() at boot.');
    }
    return _costEnforcer;
}
export function createCostEnforcer(db, eventStore, costService, killFn) {
