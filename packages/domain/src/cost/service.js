...
export class CostService {
    ...
    eventStore;
    installId;
    constructor(db, eventStore, installId) {
        this.db = db;
    ...
    async appendLedger(params) {
        ...
        logger.debug({ entryId, model: params.model, costUsd, projectId: params.projectId }, 'CostService.appendLedger: wrote entry');
    ...
    async getBudget(scope, scopeId) {
        const rows = await this.db
            .select()
            .from(costBudgets)
            .where(and(eq(costBudgets.scope, scope), scopeId !== null
            ? eq(costBudgets.scopeId, scopeId)
            : dSQL `${costBudgets.scopeId} IS NULL`, eq(costBudgets.active, true)))
            .limit(1);
        const row = rows[0];
        if (!row)
            return null;
    ...
    async setBudget(params) {
        ...
        await this.db
            ...
            .where(and(eq(costBudgets.scope, params.scope), params.scopeId != null
     ...
     */
    async summarize(projectId, sprintId) {
        const now = new Date();
        ...
        const todayRows = await this.db
            ...
            .from(costLedger)
            .where(and(eq(costLedger.projectId, projectId), ...(sprintId ? [eq(costLedger.sprintId, sprintId)] : []), dSQL `${costLedger.occurredAt} >= ${todayStart}`));
     ...
     */
    async getRunningCost(scope, scopeId) {
        let rows;
        if (scope === 'sprint') {
            rows = await this.db
...
export function registerCostService(svc) {
    _costService = svc;
}
export function getCostService() {
    if (!_costService) {
        throw new Error('STARTUP_ERROR: CostService not registered. Call registerCostService() at boot.');
    }
    return _costService;
}
export function createCostService(db, eventStore, installId) {
