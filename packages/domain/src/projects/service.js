...
import { projects } from '@orbital/db';
import { logger } from '../logger.js';
import { loadOrCreateInstall } from '../../../orchestrator/src/config/install.js';
import { CreateProjectInputSchema, UpdateProjectInputSchema, ConnectMondayInputSchema, ConnectGithubInputSchema, PROJECTS_ERROR_CODES, } from './types.js';
...
export class DefaultProjectsService {
    ...
    mondayClient;
    githubClient;
    constructor(db, eventStore, mondayClient = null, githubClient = null) {
        this.db = db;
    ...
    // create
    // -------------------------------------------------------------------------
    async create(input, actor = SYSTEM_ACTOR, tenantId = SENTINEL_TENANT) {
        ...
        if (existing) {
            throw new OrbitalError(PROJECTS_ERROR_CODES.CONFLICT_SLUG, `slug '${parsed.slug}' already in use within install`, { slug: parsed.slug });
        }
        ...
        try {
            ...
            throw new OrbitalError(PROJECTS_ERROR_CODES.INTERNAL_DB_ERROR, `INSERT project failed: ${msg}`);
        }
        if (!row) {
            throw new OrbitalError(PROJECTS_ERROR_CODES.INTERNAL_DB_ERROR, 'INSERT project returned no rows');
        }
        const ev = {
    ...
    // get
    // -------------------------------------------------------------------------
    async get(projectId, tenantId = SENTINEL_TENANT) {
    ...
    async getBySlug(slug, installId) {
        const rows = await this.db
            .select()
            .from(projects)
    ...
    // update
    // -------------------------------------------------------------------------
    async update(input, actor = SYSTEM_ACTOR, tenantId = SENTINEL_TENANT) {
        const parsed = UpdateProjectInputSchema.parse(input);
        const existing = await this.get(parsed.projectId, tenantId);
        if (!existing) {
            throw new OrbitalError(PROJECTS_ERROR_CODES.NOT_FOUND_PROJECT, `project ${parsed.projectId} not found`);
        }
        ...
        const [row] = await this.db
            ...
            .returning();
        if (!row) {
            throw new OrbitalError(PROJECTS_ERROR_CODES.INTERNAL_DB_ERROR, `UPDATE project ${parsed.projectId} returned no rows`);
        }
        const ev = {
    ...
    // archive
    // -------------------------------------------------------------------------
    async archive(projectId, actor = SYSTEM_ACTOR, tenantId = SENTINEL_TENANT) {
        const existing = await this.get(projectId, tenantId);
        if (!existing) {
            throw new OrbitalError(PROJECTS_ERROR_CODES.NOT_FOUND_PROJECT, `project ${projectId} not found`);
        }
    ...
    // connectMonday
    // -------------------------------------------------------------------------
    async connectMonday(input, actor = SYSTEM_ACTOR, tenantId = SENTINEL_TENANT) {
        ...
        if (this.mondayClient !== null) {
            try {
                ...
                throw new OrbitalError(PROJECTS_ERROR_CODES.MONDAY_CONNECT_FAILED, `Could not validate Monday board ${parsed.boardId}: ${msg}`, { board_id: parsed.boardId, cause: msg });
            }
        }
        else {
            logger.warn({ projectId: parsed.projectId, boardId: parsed.boardId }, 'ProjectsService.connectMonday: no MondayClient injected; skipping validation');
        }
        const now = new Date();
        const [row] = await this.db
            .update(projects)
            ...
            .returning();
        if (!row) {
            throw new OrbitalError(PROJECTS_ERROR_CODES.INTERNAL_DB_ERROR, `UPDATE project monday connect returned no rows`);
        }
        const ev = {
            aggregate_id: parsed.projectId,
            aggregate_type: 'install',
    ...
    // connectGithub
    // -------------------------------------------------------------------------
    async connectGithub(input, actor = SYSTEM_ACTOR, tenantId = SENTINEL_TENANT) {
        ...
        if (this.githubClient !== null) {
            try {
                ...
                if (!repo) {
                    throw new OrbitalError(PROJECTS_ERROR_CODES.GITHUB_CONNECT_FAILED, `Github repo ${parsed.owner}/${parsed.repo} not found`, { owner: parsed.owner, repo: parsed.repo });
                }
                ...
                const msg = err instanceof Error ? err.message : String(err);
                throw new OrbitalError(PROJECTS_ERROR_CODES.GITHUB_CONNECT_FAILED, `Could not validate Github repo ${parsed.owner}/${parsed.repo}: ${msg}`, { owner: parsed.owner, repo: parsed.repo, cause: msg });
            }
        }
        else {
            logger.warn({ projectId: parsed.projectId, owner: parsed.owner, repo: parsed.repo }, 'ProjectsService.connectGithub: no GithubClient injected; skipping validation');
        }
        const now = new Date();
        const [row] = await this.db
            .update(projects)
            ...
            .returning();
        if (!row) {
            throw new OrbitalError(PROJECTS_ERROR_CODES.INTERNAL_DB_ERROR, `UPDATE project github connect returned no rows`);
        }
        const ev = {
            aggregate_id: parsed.projectId,
            aggregate_type: 'install',
    ...
    async ensureDefaultProject() {
        ...
        if (!row) {
            throw new OrbitalError(PROJECTS_ERROR_CODES.INTERNAL_DB_ERROR, 'INSERT default project returned no rows');
        }
        const ev = {
            aggregate_id: projectId,
            aggregate_type: 'install',
    ...
    // -------------------------------------------------------------------------
    async backfillExistingRows(defaultProjectId) {
        const tables = [
            'epics',
            'stories',
            'sprints',
        ...
        for (const table of tables) {
            try {
                ...
                await this.db.execute(drizzleSql `UPDATE ${drizzleSql.raw(table)} SET project_id = ${defaultProjectId} WHERE project_id IS NULL`);
            }
            catch (err) {
                ...
                logger.warn({ table, err: err.message }, `ProjectsService.ensureDefaultProject: backfill skipped for ${table}`);
            }
        }
    }
}
...
// Factory
// ---------------------------------------------------------------------------
export function createProjectsService(db, eventStore, options = {}) {
    return new DefaultProjectsService(db, eventStore, options.mondayClient ?? null, options.githubClient ?? null);
}
