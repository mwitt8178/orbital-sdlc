...
import type {
  MemoryEntryRecordedPayload,
  MemoryEntryCuratedPayload,
...
export interface MemoryService {
  ...
  list(input: ListMemoryEntriesInput, tenantId?: string): Promise<{ entries: MemoryEntry[]; total: number }>
}
...
export class DefaultMemoryService implements MemoryService {
  ...
  async record(input: CreateMemoryEntryInput, actorId: string, tenantId: string = SENTINEL_TENANT): Promise<MemoryEntry> {
    ...
    if (input.tags.length > 0) {
      await this.db.insert(projectMemoryTags).values(
        input.tags.map((tag) => ({ entryId, tag })),
      )
    }

    // Insert links
    if (input.links.length > 0) {
      await this.db.insert(projectMemoryLinks).values(
        input.links.map((l) => ({
          linkId: uuidv7(),
          entryId,
          linkKind: l.linkKind,
    ...
    // Emit event
    const payload: MemoryEntryRecordedPayload = {
      entry_id: entryId,
    ...
    logger.info(
      { entryId, projectId: input.projectId, kind: input.kind },
  ...
  async update(input: UpdateMemoryEntryInput, actorId: string, tenantId: string = SENTINEL_TENANT): Promise<MemoryEntry> {
    ...
    for (const change of changes) {
      const payload: MemoryEntryCuratedPayload = {
  ...
  async supersede(entryId: string, supersededByEntryId: string, actorId: string, tenantId: string = SENTINEL_TENANT): Promise<void> {
    const existing = await this.get(entryId, tenantId)
  ...
  async get(entryId: string, tenantId: string = SENTINEL_TENANT): Promise<MemoryEntry> {
    ...
    if (!row) {
      throw new OrbitalError(
        'NOT_FOUND_MEMORY_ENTRY',
        `Memory entry ${entryId} not found.`,
        { entry_id: entryId },
        'no_retry',
      )
    }
  ...
  async list(
    ...
    if (input.search) {
      conditions.push(
        or(
          ilike(projectMemoryEntries.title, `%${input.search}%`),
          ilike(projectMemoryEntries.body, `%${input.search}%`),
        )!,
      )
    }
    ...
    const finalConditions = tagFilteredIds
      ...
      : [where]
    const finalWhere = and(...(finalConditions.filter(Boolean) as NonNullable<typeof where>[]))
...
export function createMemoryService(db: DB, eventStore: EventStore): MemoryService {
