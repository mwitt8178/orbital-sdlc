...
export const continuousFlowKickoffRule: CeremonyTriggerRule = {
  id: RULE_ID,
  description:
    'Auto-spawn a kickoff ceremony when a sprint completes, work is ready, and no planning sprint exists',
  ...
  async match(envelope: EventEnvelope, ctx: TriggerContext): Promise<CeremonySpec | null> {
    ...
    const planningCountRows = await ctx.db.execute<{ count: number }>(dSQL`
      SELECT COUNT(*)::int AS count FROM sprints WHERE status = 'planning'
    `)
    const planningCount = Number(
      (planningCountRows as unknown as Array<{ count: number }>)[0]?.count ?? 0,
    )
