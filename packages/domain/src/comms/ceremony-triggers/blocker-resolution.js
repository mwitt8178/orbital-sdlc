...
export const blockerResolutionRule = {
    id: RULE_ID,
    description: 'Schedule a blocker-resolution ceremony when a blocker has been open >=30 min without progress',
    triggers: ['BlockerRaised'],
    async match(envelope, ctx) {
        const payload = envelope.payload;
        const blockerId = payload['blocker_id'];
        if (!blockerId)
            return null;
    ...
    `);
        ...
        const raisingPersona = blocker.raising_actor && typeof blocker.raising_actor['persona_id'] === 'string'
            ? blocker.raising_actor['persona_id']
            : null;
        return {
            ceremonyType: 'ad_hoc',
            scope: {
...
//# sourceMappingURL=blocker-resolution.js.map