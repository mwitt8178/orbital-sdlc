...
export const visionDriftRule: CeremonyTriggerRule = {
  id: RULE_ID,
  description:
    'Schedule a vision drift review when >=3 retro proposals target the vision layer in 90 days',
  triggers: ['RetroProposed'],

  async match(_envelope: EventEnvelope, ctx: TriggerContext): Promise<CeremonySpec | null> {
    const rows = await ctx.db.execute<{ count: number }>(dSQL`
    ...
    if (count < VISION_PROPOSAL_THRESHOLD) return null
