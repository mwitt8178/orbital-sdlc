...
export const codeConflictRule: CeremonyTriggerRule = {
  id: RULE_ID,
  description:
    'Schedule a path-ownership ceremony when two ready tasks have overlapping declared write paths',
  ...
  async match(_envelope: EventEnvelope, ctx: TriggerContext): Promise<CeremonySpec | null> {
    ...
    return {
      ...
      invitedRoles: [conflict.persona_a, conflict.persona_b].filter(
        (v, i, a) => a.indexOf(v) === i,
      ),
    }
  },
