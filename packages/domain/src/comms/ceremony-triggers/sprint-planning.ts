...
export const sprintPlanningRule: CeremonyTriggerRule = {
  id: RULE_ID,
  ...
  async match(envelope: EventEnvelope, ctx: TriggerContext): Promise<CeremonySpec | null> {
    ...
    const sprintRows = await ctx.db.execute<{
      sprint_id: string
      status: string
      ...
      LIMIT 1
    `)
    const sprintArr = sprintRows as unknown as Array<{
      sprint_id: string
      status: string
      story_point_capacity: number | string
    }>
    const sprint = sprintArr[0]
    if (!sprint) return null
    if (sprint.status !== 'planning') return null

    const capacity = Number(sprint.story_point_capacity)

    const readySumRows = await ctx.db.execute<{ total: number }>(dSQL`
      SELECT COALESCE(SUM(story_points), 0)::int AS total
      FROM stories
      WHERE status = 'ready' AND story_points IS NOT NULL
    `)
    const totalReadyPoints = Number(
      (readySumRows as unknown as Array<{ total: number }>)[0]?.total ?? 0,
    )

    if (totalReadyPoints < capacity) return null
