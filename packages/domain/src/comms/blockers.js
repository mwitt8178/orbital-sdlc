...
import { blockers } from '@orbital/db';
 ...
 */
export const DEFAULT_RESOLVER_CHAIN = {
...
export class DefaultBlockerService {
    db;
    eventStore;
    channels;
    resolverChain;
    onRoute;
    constructor(db, eventStore, channels, options = {}) {
        this.db = db;
    ...
    async raise(params) {
        ...
        await this.db.insert(blockers).values({
            blockerId,
            raisingActor: params.raisingActor,
        ...
        const postResult = await this.channels.post(ensure.channelId, {
            postType: 'blocker',
            payload: {
    ...
    // routeToResolver
    // -------------------------------------------------------------------------
    async routeToResolver(blockerId) {
        ...
        if (blocker.state !== 'raised' &&
            blocker.state !== 'routed' &&
            blocker.state !== 'in_resolution') {
            throw new OrbitalError('CONFLICT_INVALID_STATE_TRANSITION', `routeToResolver requires state in (raised, routed, in_resolution); have ${blocker.state}`);
        }
    ...
    async resolve(params) {
        ...
        const blocker = rows[0];
        if (!blocker)
            throw new OrbitalError('NOT_FOUND_BLOCKER', `blocker ${params.blockerId} not found`);
        if (blocker.state === 'resolved' || blocker.state === 'abandoned')
            return;
        await this.db
            .update(blockers)
            .set({
    ...
    async escalate(params) {
        ...
        if (escalations) {
            await this.channels.post(escalations.channelId, {
...
// ---------------------------------------------------------------------------
export function blockerDurableChannelName(ticketId) {
