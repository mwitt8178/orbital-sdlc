...
import { channels, channelPosts, } from '@orbital/db';
...
// Implementation
// ---------------------------------------------------------------------------
export class DefaultInboxService {
    db;
    eventStore;
    ...
    // subscribe
    // -------------------------------------------------------------------------
    subscribe(channelIds, cursor, options = {}) {
        const bufferCap = Math.min(Math.max(options.bufferCap ?? DEFAULT_BUFFER_CAP, 1), MAX_BUFFER_CAP);
        const channelSet = new Set(channelIds.map((c) => String(c)));
        ...
        const handler = (event) => {
            if (event.event_type !== 'ChannelPostAdded')
                return;
            const payload = event.payload;
            const cId = payload['channel_id'];
            if (typeof cId !== 'string' || !channelSet.has(cId))
                return;
    ...
    // -------------------------------------------------------------------------
    subscribeAsStream(channelIds, cursor, options = {}) {
    ...
    // readSince
    // -------------------------------------------------------------------------
    async readSince(channelIds, cursor, options = {}) {
...
// Helpers
// ---------------------------------------------------------------------------
function envelopeToMessage(event, recipient) {
    const payload = event.payload;
