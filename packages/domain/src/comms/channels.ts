...
import type { CapabilityBundle, EventInput, Actor, ChannelId } from '@orbital/types'
import {
  channels,
  channelPosts,
...
export interface ChannelsService {
  ...
  ensureChannel(
    kind: ChannelKind,
    refId: string,
    options?: { description?: string; createdBy?: Actor },
  ): Promise<{ channelId: ChannelId; name: string; created: boolean }>
  ...
  ): Promise<Array<{ channelId: ChannelId; subscriptionId: string }>>

  /** Unsubscribe by channel id (per actor). */
  unsubscribe(
    subscriberActor: Actor,
    channelId: ChannelId,
    reason: 'task_close' | 'explicit' | 'capability_revoked',
  ...
  crossPost(params: {
    ...
    summary: string
    justification: string
  }): Promise<{ crossPostIds: string[]; derivedPostIds: string[] }>
  ...
  getByName(name: string): Promise<{ channelId: ChannelId; name: string; kind: ChannelKind } | null>

  /** Resolve a channel id to row. */
  getById(channelId: ChannelId): Promise<{ channelId: ChannelId; name: string; kind: ChannelKind } | null>
}
...
// ---------------------------------------------------------------------------
// Mention priority policy (TRD-05 §10.4)
// ---------------------------------------------------------------------------

interface MentionPriorityContext {
  postType: ChannelPostType
  authorActor: Actor
  payload: Record<string, unknown>
}

function computeMentionPriority(
  ref: MentionRef,
  ctx: MentionPriorityContext,
): number {
  // User → role: priority 1 (FR-5.19)
  if (ctx.authorActor.type === 'user' && ref.target_type === 'persona_role') return 1
  // Critical alert (regardless of mention): priority 2
  if (ctx.postType === 'alert') {
    const severity = (ctx.payload as { severity?: unknown }).severity
    if (severity === 'critical') return 2
  }
  // Budget warning system event: priority 3
  if (ctx.postType === 'system_event') {
    const eventKind = (ctx.payload as { event_kind?: unknown }).event_kind
    if (typeof eventKind === 'string' && eventKind.includes('budget_warning')) return 3
  }
...
export function canonicalChannelName(kind: ChannelKind, refId: string): string {
  switch (kind) {
    case 'ticket_durable':
...
export class DefaultChannelsService implements ChannelsService {
  ...
  async ensureChannel(
    ...
    if (kind !== 'ceremony' && !CHANNEL_NAME_REGEX.test(name)) {
      throw new OrbitalError(
        'VALIDATION_CHANNEL_NAME_INVALID',
        `channel name '${name}' fails ^[#a-z0-9_-]+$`,
      )
    }
    ...
    if (existing[0]) {
      return {
        channelId: existing[0].channelId as ChannelId,
    ...
    const scopeRef: Record<string, string> = {}
    switch (kind) {
      case 'ticket_durable':
      case 'ticket_scratch':
    ...
    try {
      await this.db.insert(channels).values({
        ...
        createdAt: new Date(),
        createdByActor: createdBy as unknown as Record<string, unknown>,
        archivedAt: null,
        schemaVersion: 1,
      })
    } catch (err: unknown) {
      ...
      if (isUniqueViolation(err)) {
        ...
        if (winner[0]) {
          return {
            channelId: winner[0].channelId as ChannelId,
            name: winner[0].name,
            created: false,
          }
        }
    ...
    return { channelId: channelId as ChannelId, name, created: true }
  }

  async bootstrapBaseline(actor: Actor = SYSTEM_ACTOR): Promise<void> {
    const baseline: Array<{ name: string; description: string }> = [
  ...
  async post(
    ...
    try {
      ...
      throw new OrbitalError(
        'VALIDATION_POST_PAYLOAD_INVALID',
        err instanceof Error ? err.message : 'invalid post payload',
      )
    }

    // Cross-post specific guard: origin must not itself be a cross_post (prevents chains).
    if (params.postType === 'cross_post') {
      const originId = (validatedPayload as { origin_post_id?: string }).origin_post_id
      if (typeof originId === 'string') {
        ...
        if (!origin) {
          throw new OrbitalError(
            ...
            `origin_post_id ${originId} does not exist`,
          )
        }
        if (origin.postType === 'cross_post') {
          throw new OrbitalError(
            'VALIDATION_INVALID_REQUEST',
            'circular cross-post: origin must not itself be a cross_post',
          )
        }
      }
    }
    ...
    // Derive author actor.
    const author: Actor = params.author ?? this.actorFromBundle(capability)
    ...
    const explicit: CrossReferenceRef[] = params.crossReferences ?? []
    const explicitKeys = new Set(explicit.map((r) => `${r.ref_type}:${r.ref_id}`))
    const allCrossRefs: Array<{ refType: string; refId: string; startOffset: number; endOffset: number }> = [
      ...explicit.map((e) => ({
        refType: e.ref_type,
        refId: e.ref_id,
        startOffset: 0,
    ...
    for (const r of allCrossRefs) {
      if (!(CROSS_REF_TYPE as readonly string[]).includes(r.refType)) {
        throw new OrbitalError(
          'VALIDATION_INVALID_REQUEST',
          `cross_reference ref_type '${r.refType}' is not in the catalog`,
        )
      }
    }
    ...
    await this.db.insert(channelPosts).values({
      ...
      authorActor: author as unknown as Record<string, unknown>,
    ...
    const resolvedCrossReferences: Array<{ crossRefId: string; refType: string; refId: string }> = []
    if (allCrossRefs.length > 0) {
      for (const r of allCrossRefs) {
        ...
        await this.db.insert(crossReferences).values({
          crossRefId,
          postId,
          refType: r.refType as 'ticket' | 'channel' | 'adr' | 'sprint' | 'epic' | 'commit' | 'ceremony' | 'defect',
          refId: r.refId,
    ...
    const resolvedMentions: Array<{ mentionId: string; targetType: string; targetRef: string; priority: number }> = []
    const mentionRefs: MentionRef[] = params.mentions ?? []
  ...
  async subscribe(
    ...
    const out: Array<{ channelId: ChannelId; subscriptionId: string }> = []
    ...
    for (const cid of channelIds) {
      ...
      const existing = await this.db
        ...
        .where(
          and(
            eq(channelSubscriptions.channelId, cid),
            dSQL`subscriber_actor::text = ${actorJson}::text`,
            dSQL`unsubscribed_at IS NULL`,
          ),
        )
        .limit(1)
      if (existing[0]) {
      ...
      await this.db.insert(channelSubscriptions).values({
        subscriptionId,
        channelId: cid,
        subscriberActor: subscriberActor as unknown as Record<string, unknown>,
  ...
  async crossPost(params: {
    ...
    if (!origin) {
      throw new OrbitalError(
        'NOT_FOUND_POST',
        `originating_post_id ${params.originatingPostId} not found`,
      )
    }
    if (origin.postType === 'cross_post') {
      throw new OrbitalError(
    ...
    if (params.artifactRef) {
      if (!(CROSS_POST_ARTIFACT_TYPE as readonly string[]).includes(params.artifactRef.type)) {
        throw new OrbitalError(
          'VALIDATION_INVALID_REQUEST',
          `artifact_ref.type '${params.artifactRef.type}' is not allowed`,
        )
      }
    }

    const originAuthorRole =
      typeof (origin.authorActor as { persona_id?: unknown })?.persona_id === 'string'
        ? ((origin.authorActor as { persona_id: string }).persona_id)
        : ((origin.authorActor as { type?: string }).type ?? 'unknown')

    const crossPostIds: string[] = []
    const derivedPostIds: string[] = []
    ...
    for (const targetName of params.targetChannelNames) {
      ...
      if (!target) {
        throw new OrbitalError(
          'NOT_FOUND_CHANNEL',
          `target channel '${targetName}' not found`,
        )
      }
      ...
      const result = await this.post(target.channelId as ChannelId, {
        postType: 'cross_post',
        payload,
      ...
      await this.db.insert(crossPosts).values({
        ...
        artifactRefType: params.artifactRef
          ? (params.artifactRef.type as 'adr' | 'sprint_commitment' | 'security_finding' | 'standup_digest' | 'retro_outcome' | 'ticket_decision')
          : null,
  ...
  async pin(params: {
    ...
    const existing = await this.db
      ...
      .where(
        and(
          eq(pinnedPosts.channelId, params.channelId),
          eq(pinnedPosts.postId, params.postId),
          dSQL`unpinned_at IS NULL`,
        ),
      )
      .limit(1)
    if (existing[0]) {
      throw new OrbitalError(
        'CONFLICT_PIN_ALREADY_ACTIVE',
        `post ${params.postId} already pinned in channel ${params.channelId}`,
      )
    }
    ...
    await this.db.insert(pinnedPosts).values({
      ...
      pinnedByActor: params.actor as unknown as Record<string, unknown>,
      capabilityId: params.capabilityId,
      pinnedAt: new Date(),
      unpinnedAt: null,
  ...
  async react(params: {
    ...
    await this.db.insert(reactions).values({
      ...
      reactorActor: params.actor as unknown as Record<string, unknown>,
    ...
    const updated = await this.db
      ...
      .where(
        and(
          ...
          eq(reactions.reactionType, params.reactionType),
          dSQL`reactor_actor::text = ${actorJson}::text`,
  ...
  async recordPresence(params: {
    ...
    const existing = await this.db
      ...
      .where(
        and(
          eq(presenceIndicators.channelId, params.channelId),
          dSQL`subscriber_actor::text = ${actorJson}::text`,
        ),
      )
      .limit(1)
    ...
    if (existing[0]) {
      ...
      await this.db.insert(presenceIndicators).values({
        presenceId: uuidv7(),
        subscriberActor: params.actor as unknown as Record<string, unknown>,
        channelId: params.channelId,
  ...
  async flushScratchChannel(taskId: string, ticketId: string): Promise<void> {
    ...
    if (!scratch) return // nothing to flush
    ...
    const lines = scratchPosts.map((p) => {
      const body =
        typeof (p.payload as { body?: unknown }).body === 'string'
          ? ((p.payload as { body: string }).body)
  ...
  async getByName(
    name: string,
  ): Promise<{ channelId: ChannelId; name: string; kind: ChannelKind } | null> {
    const rows = await this.db.select().from(channels).where(eq(channels.name, name)).limit(1)
    const r = rows[0]
    if (!r) return null
    return { channelId: r.channelId as ChannelId, name: r.name, kind: r.kind as ChannelKind }
  }
  ...
  private actorFromBundle(bundle?: CapabilityBundle): Actor {
    if (!bundle) return SYSTEM_ACTOR
    return {
      type: 'persona',
...
function extractBody(postType: ChannelPostType, payload: Record<string, unknown>): string | null {
  if (typeof payload['body'] === 'string') return payload['body']
  // Some post types use other fields as the textual body.
  if (postType === 'cross_post' && typeof payload['summary'] === 'string') return payload['summary']
 ...
 */
export async function seedChannelPostTypes(db: DB): Promise<void> {
  for (const [postType, schema] of Object.entries(POST_TYPE_SCHEMAS)) {
    const row: typeof channelPostTypes.$inferInsert = {
      postType,
