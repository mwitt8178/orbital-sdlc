 ...
 */
export function heuristicMap(schema, options) {
    ...
    const estimateCol = numberCols.find((c) => /\b(point|estimate|hours|effort|size)/i.test(c.title)) ??
        null;
    // priority_column_id (often a status with title ~ priority, sometimes a dropdown)
    const priorityCandidates = columns.filter((c) => c.type === 'status' || c.type === 'dropdown');
    const priorityCol = priorityCandidates.find((c) => /priority/i.test(c.title)) ?? null;
    // ac_column_id (long-text first; some teams use plain text)
    const longTextCols = columns.filter((c) => c.type === 'long-text');
    const acTextCol = longTextCols.find((c) => /(acceptance|criteria|\bac\b|definition.of.done|\bdod\b)/i.test(c.title)) ?? null;
    ...
    let acSubitemId = null;
    if (has_subitems && subitem_columns) {
        const candidate = subitem_columns.find((c) => (c.type === 'text' || c.type === 'long-text') &&
    ...
    // story_points_unit
    let storyPointsUnit = 'none';
    if (estimateCol) {
        if (/hour/i.test(estimateCol.title))
            storyPointsUnit = 'hours';
        else if (/point/i.test(estimateCol.title))
            storyPointsUnit = 'story_points';
        else if (/t.shirt|size/i.test(estimateCol.title))
            storyPointsUnit = 't_shirt';
        else
            storyPointsUnit = 'story_points'; // sensible default
    }
    return {
 ...
 */
export function matchLabelToState(label) {
    const l = label.toLowerCase();
    // Order matters: more specific rules first.
    if (/accept|sign.?off|approved|delivered/i.test(l))
        return 'accepted';
    if (/(in.?review|review|waiting.?for|pr.?open|qa)/i.test(l))
        return 'in_review';
    if (/(in.?progress|working|doing|wip|started)/i.test(l))
        return 'in_progress';
    if (/(ready|to.?do|todo|next|planned|prioritized|prioritised|new)/i.test(l))
        return 'ready';
    if (/(done|complete|finish|closed|merged|shipped)/i.test(l))
        return 'done';
    if (/(defect|bug|broken|stuck|blocked|fail)/i.test(l))
        return 'defective';
    if (/(backlog|icebox|idea|draft)/i.test(l))
        return 'backlog';
    return 'backlog';
}
function inferTerminology(columns, schema) {
 ...
 */
export function buildLlmUserPrompt(schema, baseline) {
...
const SYSTEM_ACTOR_FALLBACK = { type: 'system', component: 'orchestrator' };
export class DefaultBoardMappingService {
    db;
    eventStore;
    driver;
    driverPersona;
        ...
        if (this.driver) {
            try {
                const result = await this.driver.invoke({
                    ...
                    responseSchema: BoardMappingSchema,
                });
                ...
                logger.warn({ err: err.message, projectId: options.projectId }, 'BoardMappingService.propose: LLM refinement failed; falling back to heuristic');
                return baseline;
            }
        }
        return baseline;
    }
    async confirm(mapping, actor) {
        ...
        const existing = await this.db
            .select()
            .from(boardMappings)
            .where(and(eq(boardMappings.projectId, validated.project_id), eq(boardMappings.boardId, validated.board_id)))
            .limit(1);
        if (existing[0]) {
            await this.db
    ...
    async get(projectId) {
        const rows = await this.db
            .select()
            .from(boardMappings)
            .where(and(eq(boardMappings.projectId, projectId), isNotNull(boardMappings.confirmedAt)))
        ...
        if (!parsed.success) {
            logger.warn({ projectId, mappingId: row.mappingId, errs: parsed.error.message }, 'BoardMappingService.get: stored mapping failed schema validation');
            return null;
        }
        return parsed.data;
    }
...
function actorToString(actor) {
    ...
    if (actor.type === 'persona')
        return `persona:${actor.persona_id}`;
    if (actor.type === 'hook')
        return `hook:${actor.hook_id}`;
    return 'unknown';
}
...
// Factory
// ---------------------------------------------------------------------------
export function createBoardMappingService(db, eventStore, options = {}) {
    return new DefaultBoardMappingService(db, eventStore, options);
}
//# sourceMappingURL=board-mapping.js.map