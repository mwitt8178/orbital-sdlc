...
import { sprints, sprintCommitments, } from '@orbital/db';
import { tasks, taskDependencies } from '@orbital/db';
import { DEFAULT_RETRY_BUDGET, DEFAULT_TOKEN_BUDGET, DEFAULT_WALL_CLOCK_TIMEOUT_MS, } from '../../../orchestrator/src/orchestration/types.js';
import { assertAcyclic } from '../../../orchestrator/src/orchestration/dag.js';
import { logger } from '../logger.js';
import { CreateSprintInputSchema, SprintCommitmentInputSchema, isValidSprintTransition, BACKLOG_ERROR_CODES, } from './types.js';
const SYSTEM_ACTOR = { type: 'system', component: 'orchestrator' };
// ---------------------------------------------------------------------------
// Implementation
// ---------------------------------------------------------------------------
export class DefaultSprintService {
    db;
    eventStore;
    scheduler;
    pauseController;
    maxActiveSprints;
    blockerService;
    routeCallbackInstalled = false;
    constructor(db, eventStore, scheduler, pauseController, options = {}) {
        this.db = db;
        this.eventStore = eventStore;
        this.scheduler = scheduler;
        this.pauseController = pauseController;
        this.maxActiveSprints = options.maxActiveSprints ?? 3;
        if (options.blockerService)
            this.blockerService = options.blockerService;
    }
    // -------------------------------------------------------------------------
    // create
    // -------------------------------------------------------------------------
    async create(params, actor = SYSTEM_ACTOR, tenantId = SENTINEL_TENANT) {
        ...
        const sprint = await this.db.transaction(async (tx) => {
            await tx.execute(dSQL `SELECT id FROM worker_pool_state WHERE id = 1 FOR UPDATE`);
            ...
            if (activeRows.length >= this.maxActiveSprints) {
                throw new OrbitalError(BACKLOG_ERROR_CODES.CONFLICT_SPRINT_CEILING_EXCEEDED, `cannot create sprint: ceiling ${this.maxActiveSprints} reached`, { active_count: activeRows.length, ceiling: this.maxActiveSprints });
            }
            ...
            const [row] = await tx
                ...
                priorityClass: (parsed.priority_class ?? 'standard'),
                createdAt: now,
                updatedAt: now,
                schemaVersion: 1,
            })
                .returning();
            if (!row) {
                throw new OrbitalError(BACKLOG_ERROR_CODES.INTERNAL_DB_ERROR, 'INSERT sprint returned no rows');
            }
            return row;
        });
        const ev = {
    ...
    // -------------------------------------------------------------------------
    async createCommitment(input, actor = SYSTEM_ACTOR, tenantId = SENTINEL_TENANT) {
        const parsed = SprintCommitmentInputSchema.parse(input);
        const traceId = uuidv7();
        const now = new Date();
        const commitmentId = uuidv7();
        await this.db.transaction(async (tx) => {
            await tx.execute(dSQL `SELECT pg_advisory_xact_lock(hashtext(${'sprint:' + parsed.sprint_id}))`);
            const rows = await tx
                .select()
                .from(sprints)
            ...
            const sprint = rows[0];
            if (!sprint) {
                throw new OrbitalError(BACKLOG_ERROR_CODES.NOT_FOUND_SPRINT, `sprint ${parsed.sprint_id} not found`);
            }
            if (!isValidSprintTransition(sprint.status, 'ready')) {
                throw new OrbitalError(BACKLOG_ERROR_CODES.CONFLICT_INVALID_STATE_TRANSITION, `cannot commit: sprint status ${sprint.status} cannot transition to ready`, { from: sprint.status, to: 'ready' });
            }
            await tx.insert(sprintCommitments).values({
                ...
                identifiedRisks: (parsed.identified_risks ?? []),
                raisedConcerns: (parsed.raised_concerns ?? []),
    ...
    // -------------------------------------------------------------------------
    async start(sprintId, actor = SYSTEM_ACTOR, tenantId = SENTINEL_TENANT) {
        const traceId = uuidv7();
        const now = new Date();
        ...
        const { commitmentId, selectedStoryIds, sprint } = await this.db.transaction(async (tx) => {
            await tx.execute(dSQL `SELECT pg_advisory_xact_lock(hashtext(${'sprint:' + sprintId}))`);
            ...
            if (!sprint) {
                throw new OrbitalError(BACKLOG_ERROR_CODES.NOT_FOUND_SPRINT, `sprint ${sprintId} not found`);
            }
            if (!isValidSprintTransition(sprint.status, 'active')) {
                throw new OrbitalError(BACKLOG_ERROR_CODES.CONFLICT_INVALID_STATE_TRANSITION, `cannot start: sprint status ${sprint.status} cannot transition to active`, { from: sprint.status, to: 'active' });
            }
            ...
            if (!commitment) {
                throw new OrbitalError(BACKLOG_ERROR_CODES.CONFLICT_NO_COMMITMENT, `sprint ${sprintId} has no commitment`);
            }
            await tx
                .update(sprints)
        ...
        if (selectedStoryIds.length > 0) {
            ...
            if (storyTasks.length > 0) {
                ...
                const nodes = storyTasks.map((t) => ({ taskId: t.taskId, state: t.state }));
        ...
        try {
            ...
            await this.db
                ...
                .where(eq(sprints.sprintId, sprintId));
            throw new OrbitalError(BACKLOG_ERROR_CODES.CONFLICT_SPRINT_CEILING_EXCEEDED, `scheduler.addSprint rejected sprint ${sprintId}: ${err.message}`, { sprintId });
        }
        ...
        try {
            ...
            await this.db
                ...
                .where(eq(sprints.sprintId, sprintId))
                .catch(() => undefined);
            throw new OrbitalError(BACKLOG_ERROR_CODES.INTERNAL_DB_ERROR, `SprintService.start: failed to emit SprintStarted event: ${err.message}`, { sprintId });
        }
        logger.info({ sprintId, taskCount: storyTasks.length, edgeCount: edges.length }, 'SprintService.start: scheduler.addSprint dispatched');
        return { sprintId, startedAt: now };
    }
    // -------------------------------------------------------------------------
    // pause / resume — delegate to PauseController
    // -------------------------------------------------------------------------
    async pause(sprintId, reason, actor = SYSTEM_ACTOR, tenantId = SENTINEL_TENANT) {
        if (!reason.trim()) {
            throw new OrbitalError(BACKLOG_ERROR_CODES.VALIDATION_REQUIRED_FIELD_MISSING, 'reason is required');
        }
        const traceId = uuidv7();
        const now = new Date();
        await this.db.transaction(async (tx) => {
            ...
            if (!isValidSprintTransition(sprint.status, 'paused')) {
                throw new OrbitalError(BACKLOG_ERROR_CODES.CONFLICT_INVALID_STATE_TRANSITION, `cannot pause: sprint status ${sprint.status} cannot transition to paused`, { from: sprint.status, to: 'paused' });
            }
            await tx
                .update(sprints)
    ...
    async resume(sprintId, actor = SYSTEM_ACTOR, tenantId = SENTINEL_TENANT) {
        const traceId = uuidv7();
        const now = new Date();
        await this.db.transaction(async (tx) => {
            await tx.execute(dSQL `SELECT pg_advisory_xact_lock(hashtext(${'sprint:' + sprintId}))`);
            ...
            if (!isValidSprintTransition(sprint.status, 'active')) {
                throw new OrbitalError(BACKLOG_ERROR_CODES.CONFLICT_INVALID_STATE_TRANSITION, `cannot resume: sprint status ${sprint.status} cannot transition to active`, { from: sprint.status, to: 'active' });
            }
            await tx
                .update(sprints)
    ...
    // complete
    // -------------------------------------------------------------------------
    async complete(sprintId, actor = SYSTEM_ACTOR, tenantId = SENTINEL_TENANT) {
        const traceId = uuidv7();
        const now = new Date();
        const sprintRow = await this.db.transaction(async (tx) => {
            ...
            const allowedFrom = ['active', 'completing', 'paused'];
            if (!allowedFrom.includes(sprint.status)) {
                throw new OrbitalError(BACKLOG_ERROR_CODES.CONFLICT_INVALID_STATE_TRANSITION, `cannot complete: sprint status ${sprint.status} not in [${allowedFrom.join(',')}]`, { from: sprint.status, to: 'completed' });
            }
            await tx
                .update(sprints)
    ...
    // Read-side
    // -------------------------------------------------------------------------
    async list(filter = {}, tenantId = SENTINEL_TENANT) {
        const conditions = [eq(sprints.tenantId, tenantId)];
        if (filter.status)
            conditions.push(eq(sprints.status, filter.status));
        const where = and(...conditions);
        return await this.db.select().from(sprints).where(where).orderBy(dSQL `sequence DESC`);
    }
    async get(sprintId, tenantId = SENTINEL_TENANT) {
        ...
        this.blockerService.setOnRoute(async ({ resolverRole, resolverTaskId, raisingTaskId }) => {
            try {
                ...
                if (!sRows[0] || sRows[0].status !== 'active') {
                    logger.info({ sprintId: raising.sprintId, status: sRows[0]?.status }, 'SprintService.onRoute: sprint not active; skipping resolver task');
                    return;
                }
                ...
                logger.debug({ resolverTaskId, raisingTaskId, sprintId: raising.sprintId }, 'SprintService.onRoute: resolver task persisted');
            }
            catch (err) {
 ...
 */
export function priorityClassToWeight(cls) {
    if (cls === 'critical')
        return 5;
    if (cls === 'standard')
        return 3;
...
// Factory
// ---------------------------------------------------------------------------
export function createSprintService(db, eventStore, scheduler, pauseController, options = {}) {
