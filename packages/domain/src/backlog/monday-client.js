...
import { getKeychain } from '../../../orchestrator/src/capabilities/keychain.js';
import { BACKLOG_ERROR_CODES } from './types.js';
// ---------------------------------------------------------------------------
// Implementation
// ---------------------------------------------------------------------------
export class DefaultMondayClient {
    resolvedToken = null;
    apiUrl;
    maxRetries;
    baseBackoffMs;
    ...
    async resolveToken() {
        ...
        throw new OrbitalError(BACKLOG_ERROR_CODES.STARTUP_ERROR, 'MONDAY_API_TOKEN is not set; set it in env or store under keychain account monday_api_token');
    }
    // -------------------------------------------------------------------------
    ...
    async graphql(query, variables = {}) {
        ...
        for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
            try {
                ...
                if (res.status === 401 || res.status === 403) {
                    throw new OrbitalError(BACKLOG_ERROR_CODES.INTEGRATION_MONDAY_AUTH, `Monday auth failed: HTTP ${res.status}`, { status: res.status });
                }
                if (res.status === 429) {
                    // Rate limited: parse Retry-After
                    const retryAfter = parseRetryAfter(res.headers.get('retry-after'));
                    if (attempt >= this.maxRetries) {
                        throw new OrbitalError(BACKLOG_ERROR_CODES.RATE_LIMIT_MONDAY_API, `Monday rate limit exhausted after ${this.maxRetries} retries`, { retry_after_ms: retryAfter });
                    }
                ...
                if (res.status >= 500) {
                    if (attempt >= this.maxRetries) {
                        throw new OrbitalError(BACKLOG_ERROR_CODES.INTEGRATION_MONDAY_DOWN, `Monday ${res.status} after ${this.maxRetries} retries`, { status: res.status });
                    }
                    const backoff = this.computeBackoff(attempt);
                    logger.warn({ attempt, status: res.status, backoff_ms: backoff }, 'MondayClient: 5xx; retrying');
                    await this.sleepFn(backoff);
                    continue;
                }
                if (!res.ok) {
                    throw new OrbitalError(BACKLOG_ERROR_CODES.INTEGRATION_MONDAY_DOWN, `Monday request failed: HTTP ${res.status}`, { status: res.status });
                }
                ...
                if (json.errors && json.errors.length > 0) {
                    ...
                    throw new OrbitalError(BACKLOG_ERROR_CODES.INTEGRATION_MONDAY_DOWN, `Monday GraphQL errors: ${message}`, { errors: json.errors, retryable: false });
                }
                if (!json.data) {
                    throw new OrbitalError(BACKLOG_ERROR_CODES.INTEGRATION_MONDAY_DOWN, 'Monday response missing data field', { retryable: false });
                }
                return json.data;
            }
            catch (err) {
                ...
                if (err instanceof OrbitalError) {
                    if (err.code === BACKLOG_ERROR_CODES.INTEGRATION_MONDAY_AUTH ||
                        err.code === BACKLOG_ERROR_CODES.RATE_LIMIT_MONDAY_API ||
                        err.code === BACKLOG_ERROR_CODES.STARTUP_ERROR) {
                        throw err;
                    }
                    if (err.code === BACKLOG_ERROR_CODES.INTEGRATION_MONDAY_DOWN &&
                ...
                if (attempt >= this.maxRetries) {
                    throw new OrbitalError(BACKLOG_ERROR_CODES.INTEGRATION_MONDAY_DOWN, `Monday request failed after ${this.maxRetries} retries: ${err.message}`, { cause: err.message });
                }
                const backoff = this.computeBackoff(attempt);
        ...
        throw new OrbitalError(BACKLOG_ERROR_CODES.INTEGRATION_MONDAY_DOWN, `Monday request loop exited unexpectedly: ${lastErr?.message ?? 'unknown'}`);
    }
    ...
    `;
        const data = await this.graphql(query, { itemId: [itemId] });
    ...
    `;
        const data = await this.graphql(query, { boardId: [boardId] });
...
// Factory
// ---------------------------------------------------------------------------
export function createMondayClient(options = {}) {
