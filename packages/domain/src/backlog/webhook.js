 ...
 */
export function verifyMondaySignature(rawBody, signature, secret) {
 ...
 */
export function registerBacklogWebhook(app, options) {
    if (!options.secret || options.secret.length === 0) {
        throw new OrbitalError(BACKLOG_ERROR_CODES.STARTUP_ERROR, 'registerBacklogWebhook: secret is required');
    }
    app.post('/api/v1/webhooks/monday', {
        ...
        const sig = (req.headers['x-monday-signature'] ?? null);
        ...
        try {
            ...
            return reply.status(422).send({
                error: {
                    ...
                    message: `invalid webhook body: ${err.message}`,
                    trace_id: 'webhook',
                },
            });
        }
