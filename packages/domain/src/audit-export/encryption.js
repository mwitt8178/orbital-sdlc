...
export async function deriveKey(passphrase, salt) {
    return new Promise((resolve, reject) => {
        scrypt(passphrase, salt, KEY_LENGTH, { N: SCRYPT_N, r: SCRYPT_R, p: SCRYPT_P, maxmem: SCRYPT_N * SCRYPT_R * 128 * SCRYPT_P * 2 }, (err, derivedKey) => {
            if (err)
                reject(err);
            else
                resolve(derivedKey);
 ...
 */
export async function createStreamEncryptContext(passphrase) {
    const salt = randomBytes(SALT_LENGTH);
    const iv = randomBytes(IV_LENGTH);
    const key = await deriveKey(passphrase, salt);
    const cipher = createCipheriv('aes-256-gcm', key, iv, { authTagLength: TAG_LENGTH });
 ...
 */
export function finalizeStreamEncrypt(cipher) {
