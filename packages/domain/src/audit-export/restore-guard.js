 ...
 */
export async function assertDatabaseEmptyOrConfirmed(db, opts = {}) {
    if (opts.force) {
    ...
    throw new OrbitalError('CONFLICT_RESTORE_NON_EMPTY', `Database has existing data (${detail}). Use --force to overwrite.`, {
        non_empty_tables: nonEmpty,
        recovery_advice: 'Run with --force to overwrite, or manually truncate the tables before restoring.',
    }, 'no_retry');
}
/**
 ...
 */
async function countTableRows(db) {
...
//# sourceMappingURL=restore-guard.js.map