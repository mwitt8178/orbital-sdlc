 ...
 */
export function enumerateMonths(rangeStart, rangeEnd) {
    const months = [];
...
export class ExportGenerator {
    ...
    constructor(db, eventStore, queryService) {
        this.db = db;
     ...
     */
    async generateShardsParallel(req, months) {
    ...
    async generateShard(req, month) {
        ...
        const compressed = zstd.compress(jsonlBuffer, { compressionLevel: ZSTD_LEVEL });
    ...
    async packTarball(req, shards) {
        ...
        for (const shard of shards) {
            const [year, mon] = shard.month.split('-').map(Number);
        ...
        for (const shard of shards) {
            await new Promise((resolve, reject) => {
                pack.entry({ name: `events/events-${shard.month}.jsonl.zst`, size: shard.compressedBytes.length }, shard.compressedBytes, (err) => {
                    if (err)
                        reject(err);
                    else
                        resolve();
        ...
        await new Promise((resolve, reject) => {
            pack.entry({ name: 'soc2_control_mapping.json', size: controlMappingBuffer.length }, controlMappingBuffer, (err) => {
                if (err)
                    reject(err);
                else
                    resolve();
        ...
        // Add manifest.json
        await new Promise((resolve, reject) => {
            pack.entry({ name: 'manifest.json', size: manifestBuffer.length }, manifestBuffer, (err) => {
                if (err)
                    reject(err);
                else
                    resolve();
    ...
    // Helpers
    // ---------------------------------------------------------------------------
    async buildManifestBytes(req, artifacts, shards) {
        const totalEventCount = shards.reduce((acc, s) => acc + s.eventCount, 0);
        const manifest = this.manifestBuilder.build({
            exportId: req.exportId,
            installId: req.installId,
    ...
    async ensurePackageDir(exportId) {
    ...
    async failExport(exportId, errorCode, errorMessage, stage) {
        try {
            await this.db
                .update(auditExports)
    ...
    async emitProgress(exportId, percent, stage, artifactsDone, artifactsTotal, bytesWritten) {
        try {
            await this.eventStore.append({
                aggregate_id: exportId,
                aggregate_type: 'audit_export',
...
// Factory
// ---------------------------------------------------------------------------
export function createExportGenerator(db, eventStore, queryService) {
