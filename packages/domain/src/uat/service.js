...
import { uatSessions, uatAcResults, defects as defectsTable, } from '@orbital/db';
import { storyAcceptanceCriteria, stories } from '@orbital/db';
...
// Implementation
// ---------------------------------------------------------------------------
export class DefaultUATService {
    db;
    eventStore;
    defectService;
    personaOfRecord;
    constructor(db, eventStore, defectService, personaOfRecord) {
        this.db = db;
        this.eventStore = eventStore;
        this.defectService = defectService;
        this.personaOfRecord = personaOfRecord;
    }
    // -------------------------------------------------------------------------
    // startSession
    // -------------------------------------------------------------------------
    async startSession(input, userActor, tenantId = SENTINEL_TENANT) {
        ...
        if (resume_existing) {
            const resumable = await this.db
                .select()
                .from(uatSessions)
                .where(and(eq(uatSessions.ticketId, ticket_id), eq(uatSessions.tenantId, tenantId), inArray(uatSessions.state, ['started', 'in_progress'])))
            ...
            if (resumable.length > 0 && resumable[0]) {
                ...
                logger.debug({ uatSessionId: session.uatSessionId }, 'UATService.startSession: resuming existing session');
        ...
        if (storyRows.length === 0) {
            throw new OrbitalError(UAT_ERROR_CODES.NOT_FOUND_TICKET, `ticket/story ${ticket_id} not found`);
        }
        ...
        logger.info({ uatSessionId: sessionId, ticketId: ticket_id, sessionVersion, acCount: acs.length }, 'UATService.startSession: session started');
        return { session, acResults };
    }
    // -------------------------------------------------------------------------
    // markAC
    // -------------------------------------------------------------------------
    async markAC(input, userId, tenantId = SENTINEL_TENANT) {
        ...
        if (status === 'fail' && (!observed_behavior || observed_behavior.trim().length === 0)) {
            throw new OrbitalError(UAT_ERROR_CODES.VALIDATION_REQUIRED_FIELD_MISSING, 'observed_behavior is required when status is fail');
        }
        return await this.db.transaction(async (tx) => {
            // Advisory lock to serialise marks on same session
            await tx.execute(dSQL `SELECT pg_advisory_xact_lock(hashtext(${'uat:' + uat_session_id}))`);
            ...
            const [session] = await tx
                ...
                .limit(1);
            if (!session) {
                throw new OrbitalError(UAT_ERROR_CODES.NOT_FOUND_UAT_SESSION, `session ${uat_session_id} not found`);
            }
            // Guard: cannot mark after terminal
            if (['accepted', 'partially_accepted', 'rejected', 'submitted'].includes(session.state)) {
                throw new OrbitalError(UAT_ERROR_CODES.CONFLICT_INVALID_STATE_TRANSITION, `cannot mark AC on session in state ${session.state}`);
            }
            // Fetch AC result
            const [acResult] = await tx
                .select()
                .from(uatAcResults)
                .where(and(eq(uatAcResults.uatSessionId, uat_session_id), eq(uatAcResults.acId, ac_id)))
                .limit(1);
            if (!acResult) {
                throw new OrbitalError(UAT_ERROR_CODES.NOT_FOUND_AC, `AC ${ac_id} not found in session ${uat_session_id}`);
            }
            ...
            logger.debug({ uatSessionId: uat_session_id, acId: ac_id, status, priorStatus }, 'UATService.markAC: AC marked');
            return {
                ac_result_id: acResult.acResultId,
                status,
    ...
    // unmarkAC
    // -------------------------------------------------------------------------
    async unmarkAC(input, userId, tenantId = SENTINEL_TENANT) {
        ...
        return await this.db.transaction(async (tx) => {
            ...
            if (['accepted', 'partially_accepted', 'rejected', 'submitted'].includes(session.state)) {
                throw new OrbitalError(UAT_ERROR_CODES.CONFLICT_INVALID_STATE_TRANSITION, `cannot unmark AC on session in state ${session.state}`);
            }
            const [acResult] = await tx
                .select()
                .from(uatAcResults)
            ...
            if (acResult.status === 'pending') {
                ...
                const counts = await this.getCountsTx(tx, uat_session_id);
                return { ac_result_id: acResult.acResultId, status: 'pending', ...counts };
            }
            const priorStatus = acResult.status;
            // Revert to pending
            await tx
    ...
    // submit
    // -------------------------------------------------------------------------
    async submit(input, userId, tenantId = SENTINEL_TENANT) {
        ...
        return await this.db.transaction(async (tx) => {
            ...
            if (!['started', 'in_progress', 'submitted'].includes(session.state)) {
                throw new OrbitalError(UAT_ERROR_CODES.CONFLICT_INVALID_STATE_TRANSITION, `cannot submit session in state ${session.state}`);
            }
            ...
            const pendingACs = await tx
                .select()
                .from(uatAcResults)
                .where(and(eq(uatAcResults.uatSessionId, uat_session_id), eq(uatAcResults.status, 'pending')));
            if (pendingACs.length > 0) {
                throw new OrbitalError(UAT_ERROR_CODES.VALIDATION_PENDING_ACS_REMAIN, `${pendingACs.length} AC(s) still pending — mark all ACs before submitting`, { pending_count: pendingACs.length });
            }
            // Determine outcome
            const outcome = session.failCount === 0
                ? 'accepted'
            ...
            const { outcome, session, traceId: tid, now: ts } = r;
            // Fetch failed ACs for defect creation
            const failedAcResults = await this.db
                .select()
                .from(uatAcResults)
                .where(and(eq(uatAcResults.uatSessionId, uat_session_id), eq(uatAcResults.status, 'fail')));
            ...
            for (const acResult of failedAcResults) {
                ...
                const existingDefect = await this.db
                    .select()
                    .from(defectsTable)
                    .where(and(eq(defectsTable.originStoryId, session.ticketId), eq(defectsTable.originAcId, acResult.acId), inArray(defectsTable.state, ['open', 'triaged', 'assigned', 'in_progress'])))
                    .limit(1);
            ...
            if (outcome === 'accepted') {
                ...
                for (const r of allAcResults) {
                    if (r.status === 'pass')
                        passedAcIds.push(r.acId);
                    else if (r.status === 'fail')
                        failedAcIds.push(r.acId);
                }
                await this.eventStore.append({
                    aggregate_id: uat_session_id,
                    aggregate_type: 'uat_session',
            ...
            logger.info({ uatSessionId: uat_session_id, outcome, defectsCount: defectsCreated.length }, 'UATService.submit: session submitted');
            return {
                uat_session_id,
                outcome,
                pass_count: session.passCount,
    ...
    // accept
    // -------------------------------------------------------------------------
    async accept(sessionId, userId, tenantId = SENTINEL_TENANT) {
        const traceId = uuidv7();
        const now = new Date();
        const [session] = await this.db
            .select()
        ...
        if (session.state !== 'submitted') {
            throw new OrbitalError(UAT_ERROR_CODES.CONFLICT_INVALID_STATE_TRANSITION, `cannot accept session in state ${session.state} — must be submitted`);
        }
        // Guard: cannot fully accept if there are failed ACs
        if (session.failCount > 0) {
            throw new OrbitalError(UAT_ERROR_CODES.UAT_AC_NOT_MARKED, `session has ${session.failCount} failed ACs — use partialAccept or resolve failures first`, { fail_count: session.failCount });
        }
        await this.db
            .update(uatSessions)
            .set({ state: 'accepted' })
    ...
    // partialAccept
    // -------------------------------------------------------------------------
    async partialAccept(sessionId, userId, tenantId = SENTINEL_TENANT) {
        const traceId = uuidv7();
        const now = new Date();
        const [session] = await this.db
            .select()
        ...
        if (session.state !== 'submitted') {
            throw new OrbitalError(UAT_ERROR_CODES.CONFLICT_INVALID_STATE_TRANSITION, `cannot partial-accept session in state ${session.state} — must be submitted`);
        }
        await this.db
            .update(uatSessions)
    ...
    // getSession
    // -------------------------------------------------------------------------
    async getSession(sessionId, tenantId = SENTINEL_TENANT) {
        const [session] = await this.db
            .select()
            .from(uatSessions)
            .where(and(eq(uatSessions.uatSessionId, sessionId), eq(uatSessions.tenantId, tenantId)))
    ...
    // listSessions
    // -------------------------------------------------------------------------
    async listSessions(ticketId, tenantId = SENTINEL_TENANT) {
     ...
     */
    async emitResolutionVerifiedIfIterating(ticketId, uatSessionId, traceId, now) {
        ...
        const taskRow = taskRows.reduce((best, row) => {
            if (!best)
                return row;
        ...
        const [resolvedRow] = await this.db
            .select({ total: count() })
            .from(defectsTable)
            .where(and(eq(defectsTable.originStoryId, taskRow.storyId ?? ticketId), eq(defectsTable.state, 'resolved')));
    ...
    async getCountsTx(tx, sessionId) {
        const [s] = await tx
            .select()
            .from(uatSessions)
            .where(eq(uatSessions.uatSessionId, sessionId))
            .limit(1);
        if (!s)
            return { pass_count: 0, fail_count: 0, pending_count: 0 };
        return {
    ...
    buildSubmitOutput(session, existingDefects) {
        const outcome = session.failCount === 0
            ? 'accepted'
            : session.failCount === session.totalAcCount
                ? 'rejected'
...
// Factory
// ---------------------------------------------------------------------------
export function createUATService(db, eventStore, defectService, personaOfRecord) {
