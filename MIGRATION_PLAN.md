...
| `[x]` | **3.4** `packages/domain` — aggregates + services + event store | Phase 2 | **VERIFIED** physical move complete (commit `6742b3b`): 55 files / ~20k lines git-mv'd preserving history across 8 modules (events/, projects/, memory/, backlog/, uat/, audit-export/, comms/, cost/). 55 thin shims at the old orchestrator paths re-export from `@orbital/domain/<sub>.js`. Subpath `./*.js` exports + `typesVersions` declared on @orbital/domain so deep imports resolve. All 6 builds clean; cold-import passes; cdk synth + diff zero infra changes |
...
| `[skip]` | **3.7** Delete the legacy `packages/orchestrator` package | 3.6 | Reframed: with Phase 3.2 (db) and Phase 3.4 (domain) physical moves complete, packages/orchestrator now contains 55 thin shims pointing at @orbital/db / @orbital/domain PLUS the daemon-shaped modules (orchestration/, mcp/, retros/, recovery/, replay/, verifiers/, vision/) that intentionally stay in the orchestrator process. The "legacy" package is now the *daemon's* package — not a legacy at all. Boundary lint forbids API code from importing daemon-only paths. Physical deletion of the shim layer is a future cleanup once consumers migrate their `@orbital/orchestrator/...` imports to `@orbital/domain/...` directly |
