...
| `[~]` | **1.8** Reconcile deploy drift discovered in 0.2. Force `cdk synth` immediately before `cdk deploy` so the deployed template matches source | 0.2, 1.5 | Mid-flight: deploy in progress (background bg-id `bravi3wm4`). Will verify post-deploy with `aws lambda get-function-configuration --function-name orbital-mwitt-api` |
| `[~]` | **1.9** Deploy 1.5–1.8 to `mwitt` env | 1.5, 1.6, 1.7, 1.8 | Mid-flight: `cdk deploy --context env=mwitt --require-approval never --no-rollback` running in background, output streaming to `docs/deploy-logs/phase1-deploy.log` |
| `[ ]` | **1.10** End-to-end browser verification | 1.9 | Pending deploy completion |
| `[ ]` | **1.11** Provisioned Concurrency for `api-lambda` set to 2 | 1.10 | Pending |
