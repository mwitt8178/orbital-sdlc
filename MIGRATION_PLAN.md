...
|--|--|--|--|
| `[ ]` | **0.1** Checkpoint commit of current dirty tree on its own branch (`chore/pre-migration-checkpoint`) so we can roll back without losing work | none | `git status` clean on main migration branch; checkpoint commit exists with all 150+ modified/untracked files; SHA recorded below |
| `[ ]` | **0.2** Confirm AWS deploy drift root cause for the `mwitt` env. Source CDK has `ORBITAL_HOME=/tmp/.orbital`, deployed function does not. Find why (stale build artifact, hand-edited config, or CDK never re-deployed since this code landed). Document the cause | 0.1 | Written diagnosis in `docs/aws-deployment-drift.md`. Cause is one of: stale dist/, partial deploy, manual config override, missing env var injection in CDK code path. Evidence captured |
| `[ ]` | **0.3** Inventory all module-level side effects in `packages/orchestrator/src` — every file that throws, reads filesystem, spawns child processes, or constructs daemon-only singletons at import time. Output is a CSV: file, side effect, where it's invoked from import graph | 0.1 | `docs/module-import-side-effects.csv` exists with at least all known offenders (`retros/agent-org`, `orchestration/spawn`, `orchestration/worktree`, `mcp/server`, scheduler-ref, etc.) and import chain to `appRouter` |
| `[ ]` | **0.4** Snapshot current Lambda + API GW + CloudFront + Cognito config for the `mwitt` env so we have a reference of what's deployed today | 0.1 | `docs/baseline-mwitt-snapshot.json` exists with `aws lambda get-function-configuration`, `aws apigatewayv2 get-apis`, `aws cognito-idp describe-user-pool` outputs for every relevant resource |
| `[ ]` | **0.5** CI green on the checkpoint branch — `npm test` (orchestrator + ui + infra), `tsc --noEmit`, `eslint .` | 0.1 | All three commands exit 0. Output saved to `docs/preflight-ci.log` |

**Phase 0 gate**: all 0.x items checked. Migration may proceed only after this. Checkpoint SHA: `____________________` (to be filled in)
...
| | Item | Dependencies | Done when |
|--|--|--|--|
| `[ ]` | **1.1** Create `packages/api-lambda/` package with `package.json`, `tsconfig.json`, `esbuild.config.mjs`. Single entry `src/handler.ts`. Externalize `@aws-sdk/*` and `aws-xray-sdk-core` | Phase 0 | `npm run build` in `packages/api-lambda` produces `dist/handler.mjs` < 5 MB; `unzip -l` shows no `/var/task/retros/`, no `/var/task/orchestration/`, no `/var/task/mcp/`. Bundle imports verified clean by `node --eval` smoke run with HOME unset |
| `[ ]` | **1.2** Build narrow `lambdaAppRouter` in `packages/api-lambda/src/router.ts` — explicitly imports only request/response procedures: `auth, tasks, projects, memory, boards, defects, audit, comms, prs, cost, providers, team, outbox, onboarding, channel, vision (read-only)`. Excludes: `retro, replay, code_reviews, orchestration, admin (provisioning ops)` | 1.1 | `tsc --noEmit` clean; CI guard from 1.4 passes against the bundle |
| `[ ]` | **1.3** Lazy-init refactor for any dependency that the narrow router transitively touches and that has module-level side effects. No top-level construction of clients/repos that depend on filesystem or env-resolved paths | 1.2 | Audit script (1.4) reports no module-level filesystem calls in the bundle's import graph |
| `[ ]` | **1.4** CI guard script `scripts/lambda-cold-import-check.mjs` — spawns Node with `HOME=`, `USERPROFILE=`, `ORBITAL_DEPLOY_TARGET=aws`, all other required env vars unset, dynamic-imports each Lambda's bundled handler, asserts no throw. Wired into `npm run test:ci` | 1.1 | Script run produces zero crashes for `api-lambda`; script also catches a deliberately-broken handler (negative test). Output saved to `docs/cold-import-check.log` |
| `[ ]` | **1.5** New CDK construct `ApiLambdaConstruct` replaces `LambdaTrpcConstruct` for the browser path. Single Lambda, JWT authorizer at API Gateway, `/trpc/{proxy+}` routed to it | 1.1, 1.2 | `cdk synth` clean; `cdk-nag` clean; snapshot test updated and reviewed |
| `[ ]` | **1.6** Move public/anonymous procedures (`onboarding.status`, `providers.health`, anything callable pre-login) to a `/public/{proxy+}` route bound to the SAME `api-lambda` but with no authorizer. Adapter no longer branches on auth-optional | 1.5 | API Gateway has two routes: `/trpc/{proxy+}` (JWT) and `/public/{proxy+}` (none). Browser hits `/public/onboarding.status` for SetupGate. Curl probe verifies both 200 |
| `[ ]` | **1.7** Delete the 11 unused `LambdaTrpcConstruct` instantiations (`auth, tasks, memory, comms, defects, audit, prs, cost, providers, team, onboarding`) plus the now-dead per-router secret grant loop. Keep only the `install` Lambda for `/install/{proxy+}` PKI route | 1.5 | `grep -r "LambdaTrpcConstruct" infra/lib/orbital-hub-stack.ts` shows zero matches; `cdk diff` confirms 11 functions destroyed |
| `[ ]` | **1.8** Reconcile deploy drift discovered in 0.2. If CDK was correct but stale, re-deploy. If CDK code was wrong, fix it. If something hand-edited, revert | 0.2, 1.5 | `aws lambda get-function-configuration --function-name <api-lambda> --query Environment.Variables` matches CDK source line-for-line |
| `[ ]` | **1.9** Deploy 1.5–1.8 to `mwitt` env | 1.5, 1.6, 1.7, 1.8 | `cdk deploy` exits 0; CloudFormation reports UPDATE_COMPLETE; deploy log archived to `docs/deploy-logs/phase1.log` |
| `[ ]` | **1.10** End-to-end browser verification. With browser DevTools open: load CloudFront URL, `onboarding.status` returns 200, `projects.list` returns 200 (or auth-redirected), no 500s in console | 1.9 | Screenshot of network tab saved + Playwright script `tests/e2e/phase1-smoke.spec.ts` passes against deployed env |
| `[ ]` | **1.11** Provisioned Concurrency for `api-lambda` set to 2; warm-path verification | 1.10 | `aws lambda list-provisioned-concurrency-configs` shows allocated=2; warm p99 < 200 ms over 100 sequential curls |
